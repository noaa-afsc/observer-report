...
#   filter(name == "AR_3.1_rate_output_NEW_OLE_SYSTEM.rdata")
# 
# # Download the file from g-drive into local
# drive_download(
#   data_dribble,
#   path = paste0(Rdata_files_path, "AR_3.1_rate_output_NEW_OLE_SYSTEM.rdata"),
#   overwrite = T
# )
...
load(file = paste0(Rdata_files_path, "AR_3.1_rate_output_NEW_OLE_SYSTEM.rdata"))
...
cnt_incis_by_factor_group_all_categs_NEW_OLE_SYSTEM <-
...
rate_all_groupings_category_cast_1000_NEW_OLE_SYSTEM <- 
  merge(reshape2::dcast(data = rate_all_groupings_category_NEW_OLE_SYSTEM  %>% 
                          filter(!is.na(CATEGORY)),  # for the report, ONLY show factor groups that had statement occurrences.
                        formula   = CALENDAR_YEAR + COVERAGE_TYPE + VESSEL_TYPE + GEAR_TYPE + MANAGEMENT_PROGRAM_CODE + NMFS_REGION + CONFI_FLAG ~ CATEGORY,
...
rate_all_groupings_category_cast_per_assnmt_NEW_OLE_SYSTEM <- 
  merge(
    reshape2::dcast(data      = rate_all_groupings_category_NEW_OLE_SYSTEM
...
# NOTE: has not been updated for NEW_OLE_SYSTEM date as of 20240228
...
rate_all_groupings_category_all_NEW_OLE_SYSTEM <-
  rate_all_groupings_category_cast_1000_NEW_OLE_SYSTEM %>%
  inner_join(rate_all_groupings_category_cast_per_assnmt_NEW_OLE_SYSTEM %>%
...
rate_all_groupings_category_all_NEW_OLE_SYSTEM <-
  ...
  left_join(rate_all_groupings_category_all_NEW_OLE_SYSTEM %>%
...
write.csv(file = paste0(adp_yr, "_outputs/NEW_OLE_SYSTEM_data_outputs/charts_and_tables/tables/tbl_",
                        adp_yr,
                        "_rate_category_NEW_OLE_SYSTEM.csv"
),
x    = rate_all_groupings_category_all_NEW_OLE_SYSTEM %>% 
...
summ_by_subcat_current_yr_NEW_OLE_SYSTEM <-
  raw_statements_new %>%
  distinct(CATEGORY, SUBCATEGORY) %>% # have to start here and then use a LEFT JOIN back, to ensure we get ALL cateogories, even those that may not be represented in the current year.
  left_join(raw_statements_new %>%  # summarize current year statements
  ...
  left_join(rate_no_groupings_NEW_OLE_SYSTEM %>%
              filter(CALENDAR_YEAR == as.numeric(adp_yr)) %>%
              select(SUBCATEGORY, RATE_PER_ASSNMT, RATE_PER_1000_DAYS)
  ) %>%
  # tack on the current year 0 FACTOR GROUPS
  left_join(factor_groups_by_cat_NEW_OLE_SYSTEM %>%
              filter(CALENDAR_YEAR == as.numeric(adp_yr)) %>%
              select(SUBCATEGORY, PROPORT_FACTOR_GRPS_WITH_INCIS)
  )
...
summ_by_subcat_prev_yr_NEW_OLE_SYSTEM <-
  raw_statements_new %>%
  distinct(CATEGORY, SUBCATEGORY) %>% # have to start here and then use a LEFT JOIN back, to ensure we get ALL cateogories, even those that may not be represented in the current year.
  left_join(raw_statements_new %>%  # summarize current year statements
...
write.csv(file =  paste0(adp_yr, "_outputs/NEW_OLE_SYSTEM_data_outputs/charts_and_tables/tables/tbl_",
                         adp_yr,
                         "_summ_incis_by_type_NEW_OLE_SYSTEM.csv"
),
x    = summ_by_subcat_both_years_NEW_OLE_SYSTEM %>%
  mutate(CATEGORY = factor(CATEGORY, # need to re-order the levels for the final output table
                               levels = c('OBSERVER SAFETY AND WORK ENVIRONMENT',
                                          'INTERFERENCE WITH DUTIES',
                                          'OPERATIONAL REQUIREMENTS',
                                          'GEAR/EQUIPMENT REQUIREMENTS',
                                          'PERMITS/DOCUMENTS/RECORD KEEPING AND REPORTING',
...
save(list = ls(),
     file = paste0(Rdata_files_path, "AR_4.1_summary_tables_output_NEW_OLE_SYSTEM.rdata"))


# upload the .Rdata file to g-drive
googledrive::drive_upload(
  media     = paste0(Rdata_files_path, "AR_4.1_summary_tables_output_NEW_OLE_SYSTEM.rdata"),
  name      = "AR_4.1_summary_tables_output_NEW_OLE_SYSTEM.rdata",
