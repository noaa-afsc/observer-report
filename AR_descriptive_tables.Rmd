...
rm(list= ls()[!(ls() %in% c('YEAR', 'work_data', 'addl_catch_table', 'valhalla_tem'))])
...
```{r tem_summary, message=FALSE, warning=FALSE, include=FALSE, results='asis'}



# Explore the data a little:
tem_trip_counts <- valhalla_tem %>% 
  group_by(VIDEO_REVIEW) %>% 
  summarize(trip_count = n_distinct(TRIP_ID))
...
work_data2 <- work_data %>% 
  ...
  filter(!STRATA %in% c('EM_TRW_EFP_FULL', 'EM_TRW_EFP_PART')) %>% 
  mutate(STRATA_LABEL = ifelse(STRATA %in% c('EM_HAL', 'EM_POT'), 'Fixed Gear EM',
                               ifelse(STRATA %in% c('HAL', 'POT', 'TRW'), 'Observer Trip Selection',
...
Table 4-5. -- The number (n) and percent (%) of trips and metric tons (mt) and percent (%) of groundfish and halibut catch in the groundfish fisheries in the Trawl EM sampling strata in `r as.numeric(format(Sys.Date(), "%Y"))-1` by area and type of monitoring. 


```{r Table4_5_flextable, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
...
1 insert footnotes here?  About date that this review is as-of? Other?
...
rm(list= ls()[!(ls() %in% c('YEAR','work_data', 'addl_catch_table', 'counts_wide', 'bsai', 'goa', 'catch_wide', 'table4_3', 'table4_4', 'tem_summary'))])


#save('YEAR', 'work_data', 'addl_catch_table', 'counts_wide', 'bsai', 'goa', 'catch_wide', 'table4_3', 'table4_4', 'tem_summary',
