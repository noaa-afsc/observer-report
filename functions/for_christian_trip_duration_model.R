...
# Initilize: Perform the matching of ODDS+OLS with VALHALLA, and model using the simplest model possible. The results
 # of the matching are automatically assigned to the `mod_dat` object in the global environment
td_mod0 <- model_trip_duration(work.data[ADP %in% 2022:2023], use_mod = "DAYS ~ RAW")
...
# Scraping all the non-observer strata matches to their observed strata counterparts
mod_dat_copy <- copy(mod_dat)
mod_dat_copy[
][STRATA == "EM_TRW_EFP", STRATA := "TRW"
][STRATA == "EM_HAL", STRATA := "HAL"
][STRATA == "EM_POT", STRATA := "POT"
][STRATA == "ZERO", STRATA := "HAL"]
# Stratum-specific totals
mod_dat_copy[, .(DAYS = sum(DAYS), MOD = sum(MOD)), keyby = .(ADP, STRATA)]
