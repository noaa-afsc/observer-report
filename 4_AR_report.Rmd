...
#'`NEED TO ADD THE FOLLOWING FILES TO GOOGLE SHARED DRIVE`
#'*FMA Days Paid.csv*
#'*2022-FINAL-ADP_2021-11-17_i1000-o1000-seed12345.RData* 2022 ADP data
#'*2023-FINAL-ADP_2022-11-17_i1000-o1000-seed12345.RData* 2023 ADP data
#'*2024_Draft_ADP_data.rdata* `Maybe? to be able to create 2022 work.data`

# Get actual costs from spreadsheet provided by FMA (make sure spreadsheet is up-to-date!)
days_paid <- read.csv("source_data/FMA Days Paid.csv") #' *PLACEHOLDER LOAD LOCATION (changed from "data/..."*
days_paid.2022 <- filter(days_paid, Calendar == 2022)
days_paid.2023 <- filter(days_paid, Calendar == 2023)

#' `------------2022--------------`
#'*------PLACE HOLDERS TO GET CODE RUNNING-------------------------------------*
load("source_data/2022-FINAL-ADP_2021-11-17_i1000-o1000-seed12345.RData") # 2022 ADP
#'*----------------------------------------------------------------------------*
...
#' Predicted cost for the year (in days) *CHANGE TO AVAILABLE (break by stratum)*
PredictedYR.2022 <- bud_scen_lst$bud_TYFCPN$BUD$AFF_D #%>% 
...
eval.2022 <- 
  data.frame(ActualYR.2022, PredictedYR.2022) %>%
  mutate(
    # Percent of the budget used
    pct.budget.spent = round(ActualYR.2022/PredictedYR.2022, digits = 3) * 100, #' *CHANGE TO PREDICTED?*
...
#' `------------2023--------------`
#'*------PLACE HOLDERS TO GET CODE RUNNING-------------------------------------*
load("source_data/2023-FINAL-ADP_2022-11-17_i1000-o1000-seed12345.rdata") # 2023 ADP
#'*----------------------------------------------------------------------------*
...
PredictedYR.2023 <- bud_scen_lst$bud_TYFCPN$BUD$AFF_D #%>% 
               #filter(ALLO == "min_plus_opt") %>% 
               #summarise(ADP_D = mean(ADP_D)) %>% 
               #unlist() %>% 
               #as.vector()
...
# Percent of budget predicted to be spent
eval.2023 <- 
  data.frame(ActualYR.2023, PredictedYR.2023) %>%
  mutate(
    # Percent of the budget used
    pct.budget.spent = round(ActualYR.2023/PredictedYR.2023, digits = 3) * 100,
    # Percent of budget remaining
    pct.budget.remain = round(100 - pct.budget.spent, digits = 3),
    # Was it more, less, or equal to the predicted budget?
    more.or.less = ifelse(pct.budget.remain > 0, " lower than ",
...
#'sample_budget_out <- adp_out$BUD[ALLO == "min_plus_opt"] *I think this can be called directly from bud_tbl now*

# Plot effort predictions against actual
outcomes.plot.2022 <-
  #'ggplot(sample_budget_out, aes(x = ADP_D)) + *OLD CALL*
  ggplot(bud_tbl, aes(x = ADP_D)) + #' *NEW DATA CALL*
  ...
  scale_x_continuous(limits = c(2000, 3500), breaks = seq(2000, 3500, 500)) +
...
BUDGET.2022 <- bud_scen_lst$bud_TYFCPN$BUD$BUD #'dp_res$BUD$BUD *OLD CALL*
...
# Plot
costs.plot.2022 <- 
  #ggplot(sample_budget_out, aes(x = ADP_C)) +
  ggplot(bud_tbl, aes(x = ADP_C)) + #' *NEW DATA CALL*
...
# Get the budget used in the ADP
BUDGET.2023 <- bud_scen_lst$bud_TYFCPN$BUD$BUD
...
#'*------PLACE HOLDERS TO GET CODE RUNNING-------------------------------------*
load("source_data/2022-FINAL-ADP_2021-11-17_i1000-o1000-seed12345.RData") # 2022 ADP
load("source_data/2024_Draft_ADP_data.rdata") # 2022 work.data

work.data.2022 <- filter(work.data, ADP == 2022)

rm(work.data)
gc()

work.data.2022 <- mutate(work.data.2022, TRIP_TARGET_DATE = as.Date(TRIP_TARGET_DATE), 
                         LANDING_DATE = as.Date(LANDING_DATE))

work.data.2022 <- 
  work.data.2022 %>% 
  mutate(STRATA = case_match(
    STRATA,
    c("EM_POT", "EM_HAL", "EM_TRW_EFP") ~ gsub("_", " ", STRATA),
    c("HAL", "POT", "TRW") ~ paste0("OB ", STRATA),
    c("FULL", "ZERO") ~ STRATA,
    .default = "Undefined")
    )

work.data.2022 %>% distinct(STRATA) %>% arrange(STRATA)
if(any(work.data.2022$STRATA == "Undefined")) stop("Some `STRATA` are not yet defined!")

# Hard-code the 'partial' object for 2023 that is created by the ODDS pull
partial.2022 <- tibble(
  YEAR = rep(2022, times = 6),
  STRATA = c("OB POT", "OB TRW", "EM HAL", "EM TRW EFP", "EM POT", "OB HAL"),
  #Rate = c(0.1709, 0.2268, 0.3000, 0.3333, 0.3000, 0.1787),
  GEAR = c("Pot", "Trawl", "Hook-and-line", "Trawl", "Pot" , "Hook-and-line"),
  formatted_strat = c("*OB POT*", "*OB TRW*", "*EM HAL*", "*EM TRW EFP*", "*EM POT*", "*OB HAL*")
)

#'*----------------------------------------------------------------------------*

# Isolate ADP rate predictions for the chosen design
predicted.2022 <- Nnd_tbl[, lapply(.SD, mean), keyby = .(POOL, STRATA), .SDcols = c("Nh", "nh", "dh")]

predicted.2022 <- predicted.2022 %>%
...
trip.budget.table.2022 <- 
  merge(
    ...
    work.data.2022 %>%
      filter(COVERAGE_TYPE == "PARTIAL" & STRATA %in% partial.2022$STRATA & OBSERVED_FLAG == "Y") %>% #' *HAD TO CALL OBSERVER FLAG*
...
work.data.2023 <- copy(valhalla)

# We have zero-coverage trips that were monitored. Phil is working on these. We're just gonna throw them into the observer stata for now.
unique(work.data.2023[LENGTH_OVERALL < 40 & OBSERVED_FLAG == "Y", .(
  VESSEL_ID, TRIP_ID, REPORT_ID, COVERAGE_TYPE, STRATA, OBSERVED_FLAG, TRIP_TARGET_CODE, AGENCY_GEAR_CODE, REPORTING_AREA_CODE, LANDING_DATE, LENGTH_OVERALL
)])[order(TRIP_ID)]
work.data.2023[TRIP_ID %in% c(2170855, 2170858), STRATA := "POT"]
work.data.2023[TRIP_ID %in% c(2307113, 2307114), STRATA := "HAL"]

# Change date format to eliminate times and match what is in the database
work.data.2023 <- mutate(work.data.2023, TRIP_TARGET_DATE = as.Date(TRIP_TARGET_DATE), LANDING_DATE = as.Date(LANDING_DATE))

# Re-assign stratum names
unique(work.data.2023$STRATA)

work.data.2023 <- work.data.2023 %>% mutate(STRATA = case_match(
  STRATA,
  c("EM_POT", "EM_HAL", "EM_TRW_EFP") ~ gsub("_", " ", STRATA),
  c("HAL", "POT", "TRW") ~ paste0("OB ", STRATA),
  c("FULL", "ZERO") ~ STRATA,
...
work.data.2023 %>% distinct(STRATA) %>% arrange(STRATA)
if(any(work.data.2023$STRATA == "Undefined")) stop("Some `STRATA` are not yet defined!")

# Hard-code the 'partial' object for 2023 that is created by the ODDS pull
partial.2023 <- tibble(
  YEAR = rep(2023, times = 6),
  STRATA = c("OB POT", "OB TRW", "EM HAL", "EM TRW EFP", "EM POT", "OB HAL"),
  Rate = c(0.1709, 0.2268, 0.3000, 0.3333, 0.3000, 0.1787),
  GEAR = c("Pot", "Trawl", "Hook-and-line", "Trawl", "Pot" , "Hook-and-line"),
  formatted_strat = c("*OB POT*", "*OB TRW*", "*EM HAL*", "*EM TRW EFP*", "*EM POT*", "*OB HAL*")
...
# Isolate ADP rate predictions for the chosen design
predicted.2023 <- Nnd_tbl[, lapply(.SD, mean), keyby = .(POOL, STRATA), .SDcols = c("Nh", "nh", "dh")]

predicted.2023 <- predicted.2023 %>%
  mutate(STRATA = case_when(STRATA == "EM_PLK" ~ "EM TRW EFP",
                            STRATA == "TRW" & POOL == "OB" ~ "OB TRW",
                            STRATA == "POT" & POOL == "OB" ~ "OB POT",
                            STRATA == "HAL" & POOL == "OB" ~ "OB HAL",
...
trip.budget.table.2023 <- 
  merge(
    ...
    # Actual trip days by strata
    work.data.2023 %>%
      filter(COVERAGE_TYPE == "PARTIAL" & STRATA %in% partial.2023$STRATA & OBSERVED_FLAG == "Y") %>% #' *HAD TO CALL OBSERVER FLAG*
      group_by(TRIP_ID, STRATA) %>%
      summarise(dmin = min(TRIP_TARGET_DATE, LANDING_DATE, na.rm = TRUE),
                dmax = max(TRIP_TARGET_DATE, LANDING_DATE, na.rm = TRUE)) %>%
      mutate(days = ceiling(as.numeric(difftime(dmax, dmin, units = "days"))) + 1) %>%
...
# 2020-ONLY: define time periods
date_cuts <- c(as.Date(unique(partial_og$Effective_Date)), as.Date("2020-03-26"), max(odds.dat$PLANNED_EMBARK_DATE))
...
p.table <- p.table %>%            # 2020-ONLY: Making programmed rates for Periods 3 match those of period 1
  group_by(STRATA) %>%
