...
#' This means when calculating summaries for partial coverage, we need to remove EM trawl GOA
#' calculate it on it's own and then join
...
work.offload.trw_em <- work.offload %>%
  ...
  mutate(REPORT_ID = as.character(REPORT_ID),
         CV_ID = as.integer(CV_ID),
         COVERAGE_TYPE = "PARTIAL") %>%
  mutate(REPORT_ID = case_when(!is.na(T_REPORT_ID) ~ T_REPORT_ID,
                               TRUE ~ REPORT_ID)) %>%
  select(COVERAGE_TYPE, STRATA, REPORT_ID, CV_ID, OBS_SALMON_CNT_FLAG,
         OFFLOAD_TO_TENDER_FLAG, FMP, PORT_CODE) %>%
  rename(VESSEL_ID = CV_ID,
         TRIP_ID = REPORT_ID,
         OBSERVED_FLAG = OBS_SALMON_CNT_FLAG,
         TENDER = OFFLOAD_TO_TENDER_FLAG)

# Add back to work.data
work.data.partial <- work.data %>%
  filter(STRATA != "EM TRW GOA (EFP)") %>%
...
Coverage.Table.partial <- bind_rows(
  ...
  Coverage.Table.partial.total <- work.data.partial %>%
    filter(COVERAGE_TYPE == "PARTIAL" & STRATA %in% partial$STRATA) %>%
...
#' \TODO: need to address that EM trawl is deliveries not trips
Coverage.Table.TOTALS <- work.data.partial %>%
  summarize(V = length(unique(VESSEL_ID)),
            v = length(unique(VESSEL_ID[OBSERVED_FLAG == "Y"])),
            # If we want to separate by trip vs delivery
            #N = length(unique(TRIP_ID[!grepl("EM TRW", STRATA)])),
            #n = length(unique(TRIP_ID[OBSERVED_FLAG == "Y" & !grepl("EM TRW", STRATA)])),
            #Nd = length(unique(TRIP_ID[grepl("EM TRW", STRATA)])),
            #nd = length(unique(TRIP_ID[OBSERVED_FLAG == "Y" & grepl("EM TRW", STRATA)]),
            N = length(unique(TRIP_ID)),
            n = length(unique(TRIP_ID[OBSERVED_FLAG == "Y"]))
  ) %>%
  mutate(COVERAGE_TYPE = "Total",
         ...
         #Pct.obs.deliveries = format((nd/Nd) * 100, digits = 1, nsmall = 1),
...
Pollock.landing.data <- work.data %>%
  filter(STRATA %in% c("OB TRW BSAI", "OB TRW GOA", "FULL", "EM TRW BSAI (EFP)") &
  ...
  rbind(work.offload.trw_em %>%
          select(REPORT_ID = TRIP_ID, COVERAGE_TYPE, STRATA, FMP, PORT_CODE, Observed.type = OBSERVED_FLAG,
                 TENDER) %>%
