...
Unlike observed trips, the coverage rate for EM is based on information provided from the Pacific States Marine Fisheries Commission (PSMFC) that is available to analysts in the NORPAC database. In `r year`, the mean time between receipt and completion of review was 58 days for *EM HAL* and 79 days for *EM POT* (`r tbl_nums('em.times.table', display = "cite")`). This is compared to an average of 8.8 days during pre-implementation in 2016 (NMFS 2017a, p. 87).  
...
# Alternately, have separate table for Fixed-Gear EM and then OB strata in wide-format?
tb1 <- Selection.table %>%
  filter(Strata %like% "EM") %>% select(-Period)
tb2 <- Selection.table %>%
  filter(!(Strata %like% "EM")) %>% arrange(Period, Strata, `Trip disposition`) 
tb2_wide <- merge(merge(tb2 %>% filter(Period == 1) %>% select(-Period),
      tb2 %>% filter(Period == 2) %>% select(-Period), by=c("Strata", "Trip disposition")),
      tb2 %>% filter(Period == 3) %>% select(-Period), by=c("Strata", "Trip disposition")) %>%
  arrange(Strata, `Trip disposition`)

kable(tb1, 
      align="llrrrrr",
      col.names=c("Strata", "Trip disposition", "*n*", "*N*", "Actu", "Prog", "*p*"), escape=F) %>% 
      collapse_rows(1) %>%
      kable_paper()
kable(tb2_wide, 
      align=c("llrrrrrrrrrrrrrr"),
      col.names=c("Strata", "Trip disposition", rep(c("*n*", "*N*", "Actu", "Prog", "*p*"), times=3)), escape=F) %>% 
  kable_paper() %>%
  collapse_rows(1) %>%
  add_header_above(c(" "=2, "Jan. 1 - Mar. 24" = 5,  "Mar. 24 - Jun. 30" = 5, "Jul. 1 - Dec. 31" = 5))
```
...
```{r odds.by.date.plot, echo=FALSE, fig.width=7.5, fig.height=8, dpi=600, include=FALSE}
...
```
![](figures/probability_EM HAL.png)
...
```{r probability_POT, echo=FALSE, message=FALSE, warning=FALSE, fig.width=9, fig.height=10, dpi=600}
```
![](figures/probability_POT.png)

####`r fig_nums('probability_POT', caption = paste0('Probability of observing the realized or more extreme outcome (coverage rate) in a NMFS Reporting Area in the *POT* stratum. Reporting Areas where unlikely outcomes occurred are shaded in darker colors.'))`

####

```{r probability_EM POT, echo=FALSE, message=FALSE, warning=FALSE, fig.width=9, fig.height=10, dpi=600}
```
![](figures/probability_EM POT.png)
...
```{r probability_TRW, echo=FALSE, message=FALSE, warning=FALSE, fig.width=9, fig.height=10, dpi=600}
```
![](figures/probability_TRW.png)

####`r fig_nums('probability_TRW', caption = paste0('Probability of observing the realized or more extreme outcome (coverage rate) in a NMFS Reporting Area in the *TRW* stratum. Reporting Areas where unlikely outcomes occurred are shaded in darker colors.'))`
...
```{r effort_monitored_obze_HAL, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8.5, fig.height=10, dpi=600}
```
![](figures/ts_obze_HAL.png)

####`r fig_nums('effort_monitored_obze_HAL', caption = paste0('Relative concentrations of fishing effort and monitoring coverage for the observer *HAL* stratum (blue) and zero-selection pool trips that used hook-and-line gear (goldenrod). Areas with fewer than 3 distinct fishing vessels were obscured and replaced with proportions of trips that were monitored'))`
  
#####

```{r effort_monitored_obze_POT, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8.5, fig.height=10, dpi=600}
```
![](figures/ts_obze_POT.png)

####`r fig_nums('effort_monitored_obze_POT', caption = paste0('Relative concentrations of fishing effort and monitoring coverage for the observer *POT* stratum (blue) and zero-selection pool trips that used pot gear (goldenrod). Areas with fewer than 3 distinct fishing vessels were obscured and replaced with proportions of trips that were monitored'))`

```{r effort_monitored_obze_TRW, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8.5, fig.height=5, dpi=600}
```
...
```{r effort_monitored_emob_HAL, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8.5, fig.height=10, dpi=600}
```
...
```{r effort_monitored_emob_POT, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8.5, fig.height=10, dpi=600}
```
