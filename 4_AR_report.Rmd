...
In 2024, NMFS implemented an observer deployment allocation strategy of an adjusted 15% baseline, plus optimization based on discarded groundfish, Pacific halibut (*Hippoglossus stenolepis*) prohibited species catch (PSC) and Chinook salmon PSC to determine how many trips were to be monitored in each deployment stratum.\
\
...
Selection.table <- p.table %>%
  ...
  arrange(match(STRATA, strata.order)) %>% 
  mutate(new_trip_type = mosaic::derivedFactor(
    "Initial Random Selection, *a*"= Source == "ODDS random number",
...
#'* This object is also used for pollock offload summaries *
work.offload.trw_em <- work.offload %>%
  ...
  # Replace TRIP_ID with T_REPORT_ID
...
```{r data_timeliness, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE}
...
times_figure <- ggplot(data_timeliness, aes(x = data_timeliness)) +
                ...
                geom_text(data = times_smry, aes(x = median, y = Inf, label = paste(format(median, nsmall = 2))), angle = 90, hjust = 1.2, vjust = -0.55) +
                geom_text(data = times_smry, aes(x = mean, y = Inf, label = paste(format(mean, nsmall = 2))), angle = 90, hjust = 1.2, vjust = 1.35) +
...
```{r times_figure, warning=FALSE, fig.width=7.5, fig.height=6, dpi=600, eval=FALSE}
