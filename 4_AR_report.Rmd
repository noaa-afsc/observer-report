...
odds.logged <- disposition.table %>% filter(Strata == "Total") %>% 
  summarize(total = sum(`Logged (*a*)`),
            ...
            can.usr = sum(`Canceled by user (*d*)`),
...
\
The number of trips logged in the ODDS in `r year` and their dispositions is summarized in `r tbl_nums('disposition.table', display = "cite")`. Of the `r odds.logged` total trips logged, 987 were selected, and 155 were canceled: five by ODDS (0.5%) and 150 by users (15.2%). The user cancellation rate for selected trips among strata ranged from 6.9% for *EM POT* to 21.2% for *OB HAL* in `r year`.\
\
