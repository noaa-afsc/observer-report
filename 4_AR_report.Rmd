...
flextable(Selection.table.2022) %>%
  set_header_labels(values = list(
    ...
    `*p*` = "*p*-value")) %>%
  align(part = "all", align = "center") %>%
  align(j = "Trip disposition", align = "left") %>%
  bold(bold = TRUE, part = "header") %>%
  italic(j = "Strata") %>%
  merge_v(j = "Strata") %>%
  hline(i = seq(4, nrow(Selection.table.2022), 4)) %>%
  hline(i = nrow(Selection.table.2022), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  fix_border_issues()
...
flextable(Selection.table.2023) %>%
  ...
  hline(i = nrow(Selection.table.2023), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  fix_border_issues()
...
as_grouped_data(Coverage.Table.2022, groups = "Coverage") %>%
  ...
  align(j = "Realized meets expected?", align = "left") %>%
  bold(bold = TRUE, part = "header") %>%
  bold(i = ~ !is.na(Coverage)) %>%
  italic(j = "Strata", i = c(2:4, 6:12, 14)) %>%
