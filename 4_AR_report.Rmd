...
rm(work.data.all) # This is currently only used for the trawl appendix
...
# 1 full coverage (FULL)
# 5 gear and tender partial (HAL, POT - No Tender, POT - Tender, TRW - No Tender, TRW - Tender)
# 2 regulated EM (EM HAL, EM POT) + 
# 1 EM research (Zero EM Research) +
# 1 zero coverage (ZERO)
total.strat <- length(unique(work.data$STRATA))
```
...
AvailableYR <- round(BUDGET / DAILY_RATE)
  
# Predicted cost for the year (in days)
PredictedYR <- total_days_obs

# Actual cost for the year (actual number of observer days paid for)
ActualYR <- 3315.5 # Email from Lisa Thompson 3/5/20
...
# Add predicted total observed days to the sims_out object
sample_pops_out <- sample_pops_out %>% 
                   group_by(pop_num, sampling_iter) %>% 
                   mutate(total_days_obs = sum(days_obs)) %>% 
                   ungroup()

# Plot effort predictions against actual
outcomes.plot <-
  ggplot(sample_pops_out, aes(x = total_days_obs)) +
  ...
  annotate("text", x=AvailableYR + 20, y=0.002,
            ...
            col="black", angle = 90) + 
  annotate("text", x=ActualYR - 20, y=0.002,
  ...
  scale_x_continuous(limits = c(2400, 3500), breaks=seq(2400, 4000, 100)) +
  labs(y=paste("Density of simulated outcomes (", year, " ADP", ")\n", sep = ""),
       x=paste("\nTotal days used in ", year, sep = ""))
```

```{r costs.plot, include=FALSE}
# Get predicted costs from ADP
pred_costs <- sample_pops_out %>% 
              distinct(pop_num, sampling_iter, total_cost)

# Get actual costs from spreadsheet provided by FMA
days_paid <- read.csv("FMA Days Paid.csv")
days_paid <- filter(days_paid, Calendar == 2019)
...
days_paid_sum <- days_paid %>% 
                 ...
                 mutate(Total.Dollars = Total.Base.Dollars + Total.Option.Dollars + Total.Travel.Dollars)

# Avoid scientific notation
options(scipen=10000)

# Plot
costs.plot <- ggplot(pred_costs, aes(x = total_cost)) +
              ...
              annotate("text", x=BUDGET + 21000, y=0.000001, 
                       label=paste("Available Contract Budget: $", prettyNum(BUDGET, big.mark=","), sep=""), col="black", angle = 90) +
              annotate("text", x=days_paid_sum$Total.Dollars - 21000, y=0.000001, 
              ...
              theme(text = element_text(size=14),
                    ...
                    panel.grid.major = element_blank(),
                    panel.grid.minor = element_blank()) +
              scale_x_continuous(limits = c(3800000, 5200000), breaks=c(4000000, 4500000, 5000000),
                                 labels = dollar) +
              labs(y=paste("Density of simulated outcomes (", year, " ADP", ")\n", sep = ""), 
...
# Evaluate why we were off budget in 2019
# Group by strata, fmp, and target, with strata-specifc totals
trip.budget <- merge(
  # Predicted trips days by strata, fmp, and target
  plyr::rbind.fill(
    sample_pops_out %>%
      group_by(STRATA_NEW, FMP, TRIP_TARGET_CODE, pop_num, sampling_iter) %>%
      summarise(pred_days = sum(Days)) %>%
      group_by(STRATA_NEW, FMP, TRIP_TARGET_CODE) %>% 
      summarise(pred_days = round(mean(pred_days))) %>%
      arrange(STRATA_NEW, FMP, TRIP_TARGET_CODE) %>% 
      select(STRATA = STRATA_NEW, FMP, TARGET = TRIP_TARGET_CODE, pred_days)
    ,
    # Predicted totals by strata
    sample_pops_out %>%
      group_by(STRATA_NEW, pop_num, sampling_iter) %>%
      summarise(pred_days = sum(Days)) %>%
      group_by(STRATA_NEW) %>% 
      summarise(pred_days = round(mean(pred_days))) %>% 
      arrange(STRATA_NEW) %>% 
      select(STRATA = STRATA_NEW, pred_days)
  ) %>% arrange(STRATA, FMP, TARGET, pred_days)
  ,
  # Actual trip days by strata, fmp, and target
  plyr::rbind.fill(
    work.data %>%
      filter(STRATA %in% partial$STRATA) %>% 
      filter(!(STRATA %like% "EM")) %>%
      mutate(TRIP_TARGET_CODE=ifelse(!(TRIP_TARGET_CODE %in% c("P", "B", "C", "I", "S")), "Other", TRIP_TARGET_CODE)) %>% 
      mutate(TRIP_TARGET_CODE=ifelse(TRIP_TARGET_CODE %in% c("P", "B"), "Pollock", TRIP_TARGET_CODE)) %>% 
      mutate(TRIP_TARGET_CODE=ifelse(TRIP_TARGET_CODE %in% c("C"), "Pacific Cod", TRIP_TARGET_CODE)) %>% 
      mutate(TRIP_TARGET_CODE=ifelse(TRIP_TARGET_CODE %in% c("I"), "Halibut", TRIP_TARGET_CODE)) %>% 
      mutate(TRIP_TARGET_CODE=ifelse(TRIP_TARGET_CODE %in% c("S"), "Sablefish", TRIP_TARGET_CODE)) %>% 
      mutate(FMP=ifelse(FMP %in% c("BS", "AI", "BSAI"), "BSAI", FMP)) %>% 
      mutate(FMP=ifelse(TRIP_TARGET_CODE=="Sablefish", "AK", FMP)) %>% 
      group_by(TRIP_ID, STRATA, FMP, TRIP_TARGET_CODE) %>%
      dplyr::summarise(dmin = min(TRIP_TARGET_DATE, LANDING_DATE, na.rm=TRUE),
                       dmax = max(TRIP_TARGET_DATE, LANDING_DATE, na.rm=TRUE)) %>%
      mutate(days = ceiling(as.numeric(difftime(dmax, dmin, units="days")))+1) %>%
      group_by(STRATA, FMP, TARGET = TRIP_TARGET_CODE) %>%
      summarise(act_days = sum(days))
    ,
    # Actual totals by strata
    work.data %>%
      filter(STRATA %in% partial$STRATA) %>%
      filter(!(STRATA %like% "EM")) %>%
      mutate(TRIP_TARGET_CODE=ifelse(!(TRIP_TARGET_CODE %in% c("P", "B", "C", "I", "S")), "Other", TRIP_TARGET_CODE)) %>% 
      ...
      group_by(STRATA) %>% 
      summarise(act_days = sum(days)) %>%
      select(STRATA, act_days)
  ) %>% arrange(STRATA, FMP, TARGET, act_days), all = TRUE) %>% 
  # Change NAs to 0 for predicted and actual days
  mutate(pred_days = ifelse(is.na(pred_days), 0, pred_days),
         act_days  = ifelse(is.na(act_days), 0, act_days)) %>% 
  ...
  arrange(STRATA, TARGET, FMP)
  
# Format strata column  
trip.budget.table.full <- 
  full_join(trip.budget, filter(partial, !(STRATA %like% "EM")), by="STRATA") %>%
    mutate(Strata = ifelse(is.na(FMP) & is.na(TARGET), paste(formatted_strat, "Total"), formatted_strat)) %>% 
    select(Strata, FMP, TARGET, pred_days, act_days, perc)

# Bind overall total to strata-specific results
trip.budget.table.full <- 
  plyr::rbind.fill(
    trip.budget.table.full,
    trip.budget.table.full %>% 
      summarise(pred_days = sum(pred_days[!(Strata %like% "Total")]),
                act_days = sum(act_days[!(Strata %like% "Total")])) %>% 
      mutate(Strata = "Total", diff = act_days - pred_days) %>% 
      mutate(perc = format((diff)/pred_days * 100, digits = 1, nsmall = 1)) %>% 
      select(Strata, pred_days, act_days, perc)) %>% 
    mutate(FMP = ifelse(is.na(FMP), "", FMP),
           TARGET = ifelse(is.na(TARGET), "", TARGET),
           diff = act_days - pred_days) %>% 
    select(Strata,
           FMP,
           Target = TARGET,
           `Predicted number of trip days in ADP` = pred_days, 
           `Actual number of trip days` = act_days, 
           `Difference from predicted` = diff, 
           `% Difference from predicted` = perc)

# Create a version without fmp and target for the report
trip.budget.table <- trip.budget.table.full %>%
                     select(-FMP, -Target) %>% 
                     filter(Strata %like% "Total") %>% 
                     mutate(Strata = ifelse(Strata %like% "HAL" | Strata %like% "POT" | Strata %like% "TRW", substr(Strata, 1, nchar(Strata) - 6), Strata))
```
...
disposition.table <- 
  disposition %>%
  arrange(match(STRATA, c("HAL", "EM HAL", "POT - No Tender", "POT - Tender", "EM POT", "TRW - No Tender", "TRW - Tender", "Total"))) %>% 
...
logged <-
  ...
  arrange(match(STRATA, c("HAL", "EM HAL", "POT - No Tender", "POT - Tender", "EM POT", "TRW - No Tender", "TRW - Tender")))
...
for.p.wide <- merge(
  for.p.wide, 
  odds.dat %>% 
  select(STRATA, ODDS_SELECTION_PCT) %>% 
...
#### *FLAG* ##### - Adding bonferroni adjusted pvalues! *Isn't running
#p.table$`Adjusted p` <- p.adjust(p.table$`Likelihood Actual = Programmed`, method = "bonferroni")
# Cleanup
rm(for.p.wide)
...
# put columns in order and through select()
# Selection.table <- p.table %>% arrange(Source, desc(key), STRATA) %>%
#   select(Source, "Strata" = STRATA, 'Type of trip' = key, "Total Trips" = Total.n,
#          "Selected trips" = value, "Programmed Selection (%)" = ODDS_SELECTION_PCT,
#          "Actual Selection (%)" = pct, "Likelihood Actual = Programmed" = p) 

#*FLAG* Jane's alternative:
...
odds.ts.daily.cuml <- odds.dat %>% # These are all logged trips
     ...
     summarize(n = sum(random.assign, na.rm=TRUE),
               ...
               # Nra=length(na.omit(random.assign)),
               N = length(unique(TRIP_PLAN_LOG_SEQ)),  #Same as Nra which is good
     ...
     bind_rows(
          odds.dat %>% #These are the 'valid trips' only
               ...
               summarize(n = length(unique(TRIP_PLAN_LOG_SEQ[TRIP_SELECTED_OBS == "Y"])),
                         ni = length(unique(TRIP_PLAN_LOG_SEQ[!is.na(INHERIT_OBS_TRIP_SEQ)])), #Gotta put this in date format
 ...
 #           ni_pct = round((ni / N) * 100, 2),  #percent of trips in a day that are due to inherits.
...
odds.ts.daily.cuml <- transform(odds.ts.daily.cuml,
                                STRATA = factor(STRATA, levels=c("EM HAL", "EM POT", "TRW - No Tender", "TRW - Tender", "POT - No Tender", "POT - Tender", "HAL")))
...
for.plot.text <- left_join(
                 ...
                 ,
                 select(partial, STRATA, label.programmed = Rate) %>% 
...
# Lowest and highest cancellation rate/stratum for selected trips by
# strata (remove strata that you don't want to include **FLAG** will need to
# figure out if and how to incorporate EM in the future)

sel.min.canc <- disposition %>% 
                filter(STRATA %in% partial$STRATA & random.assign == "Selected") %>%
                slice(which.min(Cancellation.Pct)) %>% 
                select(STRATA, Cancellation.Pct)

sel.max.canc <- disposition %>% 
                filter(STRATA %in% partial$STRATA & random.assign == "Selected") %>%
                slice(which.max(Cancellation.Pct)) %>% 
                select(STRATA, Cancellation.Pct)
```
...
inherit.min <- logged %>% 
               slice(which.min(Pct.selected.from.inherit)) %>% 
               select(STRATA, Pct.selected.from.inherit)

inherit.max <- logged %>% 
               slice(which.max(Pct.selected.from.inherit)) %>% 
               select(STRATA, Pct.selected.from.inherit)


# Waiver impact
waiver.min <- logged %>% 
              slice(which.min(Pct.allselected.from.waiver)) %>% 
              select(Pct.allselected.from.waiver) %>% as.character()

waiver.max <- logged %>% 
              slice(which.max(Pct.allselected.from.waiver)) %>% 
              select(Pct.allselected.from.waiver) %>% as.character()
```
...
#Selection table text *FLAG* might need to do some manual tweaking on the strata
#names depending on what comes out of NORPAC each year
# selection_txt <- p.table %>% 

# FLAG - PG - STRATA rename elsewhere for 2017 AR
#   mutate(STRATA = derivedFactor('TRW' = STRATA=="Declared Gear - Trawl",
#                                 'POT' = STRATA=="Declared Gear - Pot gear",
#                                 'HAL' = STRATA=="Declared Gear - Longline")) 

#Text strings for reporting on the observered initial selection rates by strata
pct_txt <- p.table %>% 
           filter(STRATA %in% partial$STRATA & Source=="ODDS random number") %>% 
...
random_pvals <- merge(p.table, partial) %>% 
...
#rates).
final_pvals <- merge(p.table, partial) %>% 
               filter(key=="Expected") %>% 
               select(p, formatted_strat)
...
Coverage.Table.full <- # plyr::rbind.fill(
  ...
  work.data %>%
    ...
    mutate(Pct.obs= format((n/N)*100, digits=1, nsmall=1),
           ...
           Rate = 1,
           meets.expected = ifelse(as.numeric(Pct.obs)==Rate*100, "Yes", "No")) %>% 
...
Coverage.Table.partial <- plyr::rbind.fill(
  ...
  work.data %>%
    ...
    full_join(select(partial, c(STRATA, Rate)), by = "STRATA") %>% 
    group_by(COVERAGE_TYPE, STRATA) %>%
...
Coverage.Table.zero <- # plyr::rbind.fill(Coverage.Table.zero <-
    ...
    mutate(Pct.obs=format((n/N)*100, digits=1, nsmall=1),
           deployment.method = "Trip-Selection",
           Rate = 0,
           meets.expected = ifelse(as.numeric(Pct.obs)==Rate, "Yes", "No")) %>%  
    select(COVERAGE_TYPE, STRATA, deployment.method, 
           V, v, N, n, Pct.obs, Rate, meets.expected)
  #     ,
  # Totals for zero coverage category 
  # Coverage.Table.zero.total <- work.data %>%
  #   filter(STRATA %in% c("ZERO", "EM POT - No Tender", "EM POT - Tender")) %>% 
...
Coverage.Table <- plyr::rbind.fill(Coverage.Table.full, 
  ...
  select(`Coverage` = COVERAGE_TYPE, Strata = STRATA, 
         ...
         `Realized coverage` = Pct.obs,
         `95% confidence interval lower limit`= min.expected,
         `95% confidence interval upper limit` = max.expected, 
...
trips.within.cells <- work.data %>%
  ...
  filter(STRATA %in% partial$STRATA & SOURCE_TABLE=='Y') %>%  
  distinct() %>% 
...
strata.cell.dat <- strata.cell.dat %>%
  mutate(
    ...
    Hyper.P.0 = round(1-phyper(q=0, 
                               ...
                               lower.tail = FALSE, log.p = FALSE),6),
...
strata.cell.dat <- full_join(strata.cell.dat, partial, by="STRATA")
```
                 
```{r cell.pot, message=FALSE, warning=FALSE, include=FALSE}
# Reorder strata for plotting
strata.cell.dat <- transform(strata.cell.dat, STRATA=factor(STRATA,levels=c("EM HAL", "EM POT", "POT - No Tender", "POT - Tender", "TRW - No Tender", "TRW - Tender", "HAL")))
...
#coverage stratum. Each point on a plot represents a NMFS Area. The smaller the
#point, the more unusual the result.
#Note-  each point is a NMFS area, vessels 
# Remove HAL - Tender stratum, since no trips were observed in 2017
cell.plot <- ggplot(filter(strata.cell.dat), 
  ...
  # scale_colour_manual('Probability of Observed Sample Units\nor a More Extreme Value', 
  #                     values=c(  "black", 'grey25', 'grey50', 'grey75','grey80',  "grey90" )) +
  # scale_fill_manual('Probability of Observed Sample Units\nor a More Extreme Value', 
  #                   values=c(  "black", 'grey25', 'grey50', 'grey75','grey80',  "grey90" )) +
  # scale_size_discrete('Probability of Observed Sample Units\nor a More Extreme Value',
  #                     #range = c(1.5,8)) +
  #                     range = c(1.5,5)) +
...
#length(strata.cell.dat$P_extreme_more_cat)
```
...
for (i in as.character(partial$STRATA)) {
  ...
  # Save plot  
  png(paste0("probability_", STRAT, ".png"), height = 5,
...
####`r tbl_nums('Coverage.Table', caption = paste0('Number of total vessels (V), sampled vessels (v), total trips (N), sampled trips (n) for each stratum in ', year, '. The expected coverage and 95% confidence interval columns are expressed as percentages of the total number of trips taken within each stratum.'))`  

```{r Coverage.Table, echo=FALSE, tab.cap=""}
Coverage.Table[c(5:8)] <- lapply(Coverage.Table[,5:8], prettyNum, trim=TRUE, big.mark=",")

kable(Coverage.Table)
```
