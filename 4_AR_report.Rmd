...
gdrive_download("source_data/2_AR_data.Rdata", gdrive_set_dribble("Projects/AnnRpt-Deployment-Chapter"), ver = 4)
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
#' Load data and set parameters
load("source_data/2_AR_data_v004.RData") # This includes the .Random.seed set in 1_AR_data.R
...
#'[TODO: update strata - EFP split by FMP now, fixed gears merged] Set order of strata
strata.order <- c("FULL", "EM TRW BSAI (EFP)", "OB FIXED BSAI", "OB FIXED GOA", "OB TRW BSAI", "OB TRW GOA", "EM FIXED BSAI", "EM FIXED GOA", "EM TRW GOA (EFP)", "ZERO")
...
# 4 partial coverage observed (OB FIXED BSAI, OB FIXED GOA, OB TRW BSAI, OB TRW GOA)
# 3 partial coverage EM (EM FIXED BSAI, EM FIXED GOA, EM TRW EFP[GOA]) + 
...
PredictedYR <- bud_scen_lst$bud_TYFCPN$BUD$AFF_D
...
eval <- data.frame(ActualYR, AvailableYR) %>%
  mutate(
    # Percent of the budget used
    pct.budget.spent = round(ActualYR/AvailableYR, digits = 3) * 100,
...
#' *Table 3-1 : Comparison between predicted and actual trip days for partial coverage strata in 2024.*
...
Coverage.Table.zero <-
  work.data %>%
    filter(STRATA %in% c("ZERO", "Zero EM Research")) %>% #' *NO ZERO EM Research in 2024*
    mutate(STRATA = as.character(STRATA)) %>% 
...
```{r perm_results_2022, set.seed(49), message=FALSE, warning=FALSE, include=FALSE}
...
assign(paste(YN_var, "YN_perm.2022", sep = "."), data.frame(perm[1]))
assign(paste(YN_var, "summary.2022", sep = "."), data.frame(perm[3]))
...
OBSERVED_FLAG.summary <- arrange(OBSERVED_FLAG.summary.2022, match(STRATA, c("OB HAL", "EM HAL", "OB POT", "EM POT", "OB TRW", "EM TRW EFP")), variable)
perm_sum <- filter(OBSERVED_FLAG.summary.2022, STRATA != "EM TRW EFP")
...
OBSERVED_FLAG.results <- rbind(
  OBSERVED_FLAG.results %>% 
    ...
    mutate(across(unique(OBSERVED_FLAG.YN_perm.2022$variable), ~ format(.x, nsmall = 3, digits = 3)))
  ,
  OBSERVED_FLAG.results %>% 
    filter(Metric %in% c("p-value")) %>% 
    # Test against Bonferroni-adjusted alpha (0.05/ number of metrics)
    mutate(across(unique(OBSERVED_FLAG.YN_perm.2022$variable), ~ ifelse(as.numeric(.x) < 0.05/length(unique(OBSERVED_FLAG.YN_perm.2022$variable)), paste0(format(.x, nsmall = 3, digits = 3), "*"), 
...
```{r perm_results_2023, set.seed(49), message=FALSE, warning=FALSE, include=FALSE}
...
```{r trip.budget.tbl.2023, tab.cap=""}
...
```{r disposition.table.2023, tab.cap=""}
...
flextable(logged.table.2023) %>%
  ...
  bold(bold = TRUE, part = "header") %>%
  merge_v(j = 1) %>%
  hline(i = c(3, 5, nrow(logged.table.2023)), border = fp_border(width = 1)) %>%
  surround(part = "header", border.top = fp_border(width = 1), border.bottom = fp_border(width = 1)) %>%
  fix_border_issues() %>%
...
```{r Selection.table.2023, tab.cap=""}
...
```{r Pollock.observed.table.2023, tab.cap=""}
...
flextable(Pollock.observed.table.2023) %>%
  ...
  hline(i = c(4, 9)) %>%
  hline(i = nrow(Pollock.observed.table.2023), border = fp_border(width = 1)) %>%
  surround(part = "header", border.top = fp_border(width = 1), border.bottom = fp_border(width = 1)) %>%
  fix_border_issues() %>%
  width(j = 1, 0.81, unit = "in") %>%
...
```{r Pollock.efp.table.2023, tab.cap=""}
...
flextable(Pollock.efp.table.2023) %>%
  ...
  merge_v(j = c("FMP", "Coverage category")) %>%
  hline(i = c(4, 9)) %>%
  hline(i = nrow(Pollock.efp.table.2023), border = fp_border(width = 1)) %>%
  surround(part = "header", border.top = fp_border(width = 1), border.bottom = fp_border(width = 1)) %>%
  fix_border_issues() %>%
  width(j = 1, 0.81, unit = "in") %>%
  width(j = 2, 1.39, unit = "in") %>%
...
`r tbl_nums('perm.summary.table.2023', caption = paste0('Results of permutation tests between monitored and unmonitored trips in the ', year, ' trip-selection strata. OD: Observed difference (monitored - unmonitored). Observed and unobserved columns are in units of trips. Statistically significant results are in bold italics.'))`  

```{r perm.summary.table.2023, tab.cap=""}
flextable(OBSERVED_FLAG.results_2023) %>%
  bold(bold = TRUE, part = "header") %>%
  italic(j = 1, part = "body") %>%
  hline(i = seq(3, nrow(OBSERVED_FLAG.results_2023), 3)) %>%
  surround(part = "header", border.top = fp_border(width = 1), border.bottom = fp_border(width = 1)) %>%
   mk_par(j = 4, i = seq(3, nrow(OBSERVED_FLAG.results_2023), 3), part = "body",
         value = as_paragraph(as_i("p"), "-value")) %>%
  ...
  bold(j = c(6, 10), i = 1:3, bold = TRUE, part = "body") %>%
  bold(j = 8, i = 4:6, bold = TRUE, part = "body") %>%
  bold(j = 9, i = 13:15, bold = TRUE, part = "body") %>%
  italic(j = c(6, 10), i = 1:3, part = "body") %>%
  italic(j = 8, i = 4:6, part = "body") %>%
  italic(j = 9, i = 13:15, part = "body")
```
...
```{r outcomes.plot.2023, warning=FALSE, fig.width=7.5, fig.height=9, dpi = 600}
...
```{r odds.by.date.plot.2023, fig.width=7.5, fig.height=8, dpi=600}
...
```{r Temporal.plot.2023, warning=FALSE, fig.width=7.5, fig.height=8, dpi=600}
