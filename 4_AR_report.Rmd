...
eval.2022 <- 
  data.frame(ActualYR.2022, AvailableYR.2022) %>% #' *CHANGED TO AVAILABLE FROM PREDICTED*
  mutate(
...
# Percent of budget predicted to be spent
eval.2023 <- 
  data.frame(ActualYR.2023, AvailableYR.2023) %>%
  mutate(
    # Percent of the budget used
...
rm(bud_scen_lst, Nnd_tbl, adj_tbl, bud_tbl, days_paid)
```
...
#'sample_budget_out <- adp_out$BUD[ALLO == "min_plus_opt"] *Called directly from bud_tbl now*

# Plot effort predictions against actual
outcomes.plot.2022 <-
  #'ggplot(sample_budget_out, aes(x = ADP_D)) + *OLD CALL*
  ggplot(bud_tbl.2022, aes(x = ADP_D)) + #' *NEW DATA CALL*
...
outcomes.plot.2023 <-
  ggplot(bud_tbl.2023, aes(x = ADP_D)) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 20, alpha = 0.2, fill = "blue", col = "cornflowerblue") +
...
BUDGET.2022 <- bud_scen_lst.2022$bud_TYFCPN$BUD$BUD #'dp_res$BUD$BUD *OLD CALL*
...
costs.plot.2022 <- 
  ...
  ggplot(bud_tbl.2022, aes(x = ADP_C)) + #' *NEW DATA CALL*
...
# Plot
costs.plot.2023 <- 
  ggplot(bud_tbl.2023, aes(x = ADP_C)) +
  geom_histogram(aes(y = ..density..), binwidth = 20000, alpha = 0.2, 
                 fill = "green", col = "darkseagreen2") +
...
predicted.2022 <- Nnd_tbl.2022[, lapply(.SD, mean), keyby = .(POOL, STRATA), .SDcols = c("Nh", "nh", "dh")]
...
# Isolate ADP rate predictions for the chosen design
predicted.2023 <- Nnd_tbl.2023[, lapply(.SD, mean), keyby = .(POOL, STRATA), .SDcols = c("Nh", "nh", "dh")]
