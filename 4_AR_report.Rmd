...
work.data.2023 <- filter(work.data, ADP == 2023)

work.data <- rbind(
  work.data.2022[COVERAGE_TYPE == "PARTIAL", -c("CVG_NEW", "STRATA_NEW", "PORT_NEW", "P_STRATA")], 
  work.data.2023[COVERAGE_TYPE == "PARTIAL"])

source("functions/for_christian_trip_duration_model.R") # Match OLS and VALHALLA observer days (actual)
...
obs_act_days <- mod_dat_copy[, .(DAYS = sum(DAYS), MOD = sum(MOD)), keyby = .(ADP, STRATA)]

# Format work.data
work.data <- work.data %>% 
  mutate(work.data, TRIP_TARGET_DATE = as.Date(TRIP_TARGET_DATE), 
...
work.data %>% distinct(STRATA) %>% arrange(STRATA)
if(any(work.data$STRATA == "Undefined")) stop("Some `STRATA` are not yet defined!")
...
obs_act_days.2022 <- filter(obs_act_days, ADP == 2022)
...
obs_act_days.2023 <- filter(obs_act_days, ADP == 2023)
