...
# For.ts.temporal.plot[For.ts.temporal.plot$p=="0.000*", "p"] <- "< 0.001*"
...
Temporal.plot <- ggplot(For.ts.temporal.plot, aes(x=Date)) +
  # Add waiver period
  geom_rect(data=filter(For.ts.temporal.plot, STRATA %in% c("HAL", "POT", "TRW")), aes(xmin=as.Date("2020-03-26"), xmax = as.Date("2020-07-01"), ymin=0, ymax=Inf), alpha = 0.005, col = "gray") +
...
# strata.cell.dat.
prob.bynumber.plot <- ggplot(strata.cell.dat, aes(y=Hyper.P.0, x=totl.in.cell)) +
                      geom_point(size=3) +
                      geom_line(size=1) + 
                      facet_wrap(STRATA~., ncol = 2) +
                      xlab('\nNumber of Trips') +
                      ylab('Probability of No Observed Trips\n') +
                      # This will cut out points with > 90 trips in order to focus on 
                      # areas that have some probability of getting no monitored trips
                      xlim(0, 90) +
                      fig.theme
...
* Continue to include an evaluation of observer effects in pelagic and non-pelagic trawl within the *TRW* stratum.  
...
####`r tbl_nums('Pollock.observed.table', caption = "The number of *TRW* pollock deliveries by port and coverage category.")`  
...
####`r tbl_nums('Pollock.efp.table', caption = "The number of *EM TRW EFP* pollock deliveries by port and coverage category.")`  
