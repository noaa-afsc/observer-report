...
flextable(trip.budget.table) %>%
  ...
  hline(i = which(diff(as.numeric(as.factor(substr(trip.budget.table$STRATA, 1, 2)))) != 0)) %>%
...
flextable(logged.table) %>%
  ...
  italic(j = 1, i = 1:6) %>%
  bold(bold = TRUE, part = "header") %>%
  hline(i = which(diff(as.numeric(as.factor(substr(logged.table$Strata, 1, 2)))) != 0))
  flextable::surround(part = "header", border.top = fp_border(width = 1), border.bottom = fp_border(width = 1)) %>%
  fix_border_issues() %>%
  width(j = 1, 1.23, unit = "in") %>%
...
as_grouped_data(Coverage.Table, groups = "Coverage") %>%
  ...
  flextable::surround(part = "header", border.top = fp_border(width = 1)) %>%
  flextable::surround(i = ~ !is.na(Coverage), border.top = fp_border(width = 1), border.bottom = fp_border(width = 1)) %>%
...
flextable(Pollock.observed.table) %>%
  ...
  hline(i = which(Pollock.observed.table$FMP %like% "total")) %>%
  hline(i = which(Pollock.observed.table$FMP %like% "total") - 1, border = fp_border(style = "dashed")) %>%
  hline(i = nrow(Pollock.observed.table), border = fp_border(width = 1)) %>%
  flextable::surround(part = "header", border.top = fp_border(width = 1), border.bottom = fp_border(width = 1)) %>%
  fix_border_issues() %>%
...
flextable(Pollock.efp.table) %>%
  ...
  bold(bold = TRUE, part = "header") %>%
  merge_v(j = c("FMP", "Coverage category")) %>%
  hline(i = which(Pollock.efp.table$FMP %like% "total")) %>%
  hline(i = which(Pollock.efp.table$FMP %like% "total") - 1, border = fp_border(style = "dashed")) %>%
  hline(i = nrow(Pollock.efp.table), border = fp_border(width = 1)) %>%
  flextable::surround(part = "header", border.top = fp_border(width = 1), border.bottom = fp_border(width = 1)) %>%
  fix_border_issues() %>%
  width(j = 1, 0.81, unit = "in") %>%
  width(j = 2, 1.39, unit = "in") %>%
