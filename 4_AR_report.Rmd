...
# Assuming one fish ticket per delivery
...
work.offload.trw_em <- work.offload %>%
  ...
  mutate(TRIP_ID = case_when(!is.na(T_REPORT_ID) ~ T_REPORT_ID,
                               TRUE ~ TRIP_ID)) %>%
...
Coverage.Table.partial <- bind_rows(
  ...
  Coverage.Table.partial.total <- work.data.offload %>%
    filter(COVERAGE_TYPE == "PARTIAL" & STRATA %in% partial$STRATA) %>%
    filter(STRATA != "EM TRW GOA (EFP)") %>% #'* 2024 AR only (until VALHALLA fixed) *
    summarize(V = length(unique(VESSEL_ID)),
              v = length(unique(VESSEL_ID[OBSERVED_FLAG == "Y"])),
              N = length(unique(TRIP_ID)),
              n = length(unique(TRIP_ID[OBSERVED_FLAG == "Y"]))) %>%
...
Pollock.landing.data <- work.data %>%
  ...
  # Check for non-pollock EM trawl offloads
  {
    ...
    if (non_pollock_count$`nrow(filtered_data)` > 0) {
      print(paste(non_pollock_count$`nrow(filtered_data)`, "EM trawl offloads are non-pollock"))
    } else {
      print("All EM trawl deliveries are pollock")
    }
    .} %>%
  ...
  {if (any(duplicated(.$REPORT_ID))) {
    print("Duplicate REPORT_IDs found")
  } else {
    print("No duplicate REPORT_IDs")
  }
    .} %>%
  ...
  rbind(
    work.offload.trw_em %>%
      filter(STRATA == "EM TRW GOA (EFP)") %>%
          select(REPORT_ID = TRIP_ID, COVERAGE_TYPE, STRATA, FMP, PORT_CODE,
                 Observed.type = OBSERVED_FLAG, TENDER) %>%
      #'*2024 AR* incorrect assignment of PORT_CODE for 1 tender delivery
          mutate(PORT_CODE = case_when(REPORT_ID == "37252024-09-10" ~ "AKU",
                                       TRUE ~ PORT_CODE)) %>%
          distinct() %>%
          mutate(Observed.type = if_else(Observed.type == "Y", "Observed", "Not_Observed"),
                 COVERAGE_TYPE = case_when(grepl("EM TRW GOA", STRATA)
                                           & TENDER == "Y"
                                           & COVERAGE_TYPE == "PARTIAL" ~ "Partial (tender)",
                                           grepl("EM TRW GOA", STRATA)
                                           & TENDER == "N"
                                           & COVERAGE_TYPE == "PARTIAL" ~ "Partial (non-tender)",
                                           TRUE ~ COVERAGE_TYPE)),
        fill = TRUE) %>%
...
Pollock.observed.table <- Pollock.observed %>% 
  ...
  select(`FMP` = FMP, 
         ...
         `Port` = PORT_CODE, 
         `Total offloads (*N*)` = N.pollock,
         `Observed offloads (*n*)` = N.pollock.obs,
         `% observed` = pct.obs)
