...
Coverage.Table.full <- plyr::rbind.fill(
  # Coverage rates for vessels in the full coverage category (both regulatory full
...
Coverage.Table.partial <- plyr::rbind.fill(
  ...
  work.data %>% 
    ...
    #ungroup() %>% 
    #left_join(partial[, .(STRATA, Rate)], on = "STRATA") %>% 
    ...
    summarize(V = length(unique(VESSEL_ID)), #[START >= Effective_Date & START <= Expire_After])),
              v = length(unique(VESSEL_ID[OBSERVED_FLAG == "Y"])), #[START >= Effective_Date & START <= Expire_After & OBSERVED_FLAG == "Y"])),
              N = length(unique(TRIP_ID)), #[START >= Effective_Date & START <= Expire_After])),
              n = length(unique(TRIP_ID[OBSERVED_FLAG == "Y"])), #[START >= Effective_Date & START <= Expire_After & OBSERVED_FLAG == "Y"])),
              Rate = unique(Rate)) %>%
    mutate(Pct.obs = format((n/N) * 100, digits = 1, nsmall = 1),
           deployment.method = "Trip-Selection") %>% 
    group_by(COVERAGE_TYPE, STRATA) %>%
