...
In the fixed-gear EM fixed-gear strata, fishing effort by number of trips was `r trip.budget.emfg_total$perc.N`% higher than anticipated but the number of sea days reviewed was `r abs(trip.budget.emfg_total$perc)`% lower than predicted. 
...
\
In combination across all strata, coverage levels and fishery monitoring tools,`r Coverage.Table %>% filter(Strata == "Total") %>% pull(n) %>% prettyNum(big.mark = ",")` trips/deliveries (`r Coverage.Table %>% filter(Strata == "Total") %>% mutate(round(100 * n/N,2) ) %>% pull()`%) and `r Coverage.Table %>% filter(Strata == "Total") %>% pull(V) %>% prettyNum(big.mark = ",")` vessels (`r Coverage.Table %>% filter(Strata == "Total") %>% mutate(round(100 * v/V,2) ) %>% pull()`%) were successfully monitored among all fishing in the Federal fisheries off Alaska in `r year` (`r tbl_nums('Coverage.Table', display = "cite")`). This compares to a total of 3,780 trips/deliveries (43.7%) and 463 vessels (50.2%) monitored in `r year - 1` (AFSC and AKRO `r year`).\
\
...
\
Evaluations for the full coverage category and zero-selection pool are straightforward --- either the coverage achieved was equal to 100% or 0%, respectively, or it was not. The program achieved `r Coverage.Table %>% filter(Strata == "Full") %>% pull(Realized) %>% trimws()`% coverage in its full coverage category (`r tbl_nums('Coverage.Table', display = "cite")`). One trip weas not monitored in the full coverage category (**add explanation here**). The program achieved compliance with the *Zero coverage* stratum (`r tbl_nums('Coverage.Table', display = "cite")`).\
\
...
\
Proximity indices were calculated for each stratum to evaluate whether coverage met expectations overall (`r fig_nums('plt.proximity.stratum', display = "cite")`) for the `r year` ADP (NMFS 2023b).\
\
