...
odds.dat <- odds.dat %>% mutate(
  random.test = case_when(
    ...
    # Trip was intended to be monitored but was waived
    released == 1 ~ 1,
...
#' Check whether the monitoring outcome matches what was determined by random selection, i.e., trips that were not monitored when they were supposed to be or vice versa.
...
if(year == 2024) {
  cat(paste("2024 only: hard-coding releases for two trips that failed the `random.test.result` due to three-in-a-row 
            release not populating correctly."))
  setDT(odds.dat)
