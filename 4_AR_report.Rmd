...
partial_time <- partial %>% 
                ...
                mutate(Effective_Date = as.Date(Effective_Date), Expire_After = as.Date(c("2020-12-31", "2020-12-31", "2020-12-31", "2020-03-25", "2020-03-25", "2020-03-25"))) %>% 
...
partial_time <- rbind(partial_time,
                      ...
                      mutate(Effective_Date = as.Date("2020-03-26"), Expire_After = as.Date("2020-06-30")),
                      partial_time %>% 
                      filter(STRATA %in% c("HAL", "POT", "TRW")) %>% 
                      mutate(Effective_Date = as.Date("2020-07-01"), Expire_After = as.Date("2020-12-31")))
...
#' FIXME - *Make this annotation more clear what it's doing*
# Perform a check on the random number process in ODDS (flag for whether a trip was selected due to random selection or not)
# Trips outside of trip-selection do not have random numbers in odds.odds_trip_stratas (e.g., voluntary 100%)
...
# Use output table to assess any flagged trips and make corrections as necessary
# Also convert PLANNED_EMBARK_DATE to as.Date(), which is easier to work with than POSIXct
# FIXME   There was a trip in 2023 with RANDOM_NUMBER_USED == 1 with ODDS_SELECTION_PCT == 100 that appears to have been 
# monitored in spite of technically failing the RN check. Added | (ODDS_SELECTION_PCT == 100) to handle this until ODDS
# logic can remedy this. 2024-Mar-29 Geoff Mayhew sent e-mail to Andy Kingham to address this issue.
odds.dat <- odds.dat %>% mutate(
  ...
  random.assign = case_when(
    ...
    !is.na(RANDOM_NUMBER_USED) & (RANDOM_NUMBER_USED < ODDS_SELECTION_PCT / 100) | (ODDS_SELECTION_PCT == 100) ~ 1,
    !is.na(RANDOM_NUMBER_USED) & (RANDOM_NUMBER_USED >= ODDS_SELECTION_PCT / 100) ~ 0
  )
)
...
odds.dat <- mutate(odds.dat, random.test = 0) %>% 
            mutate(random.test = ifelse(TRIP_MONITOR_CODE %in% c("ND", "RO", "OA", "RL") & !(STRATA_CODE %in% c(96, 98)), 1, random.test)) %>% 
            mutate(random.test = ifelse(!is.na(RELEASE_STATUS_DESCRIPTION), 1, random.test)) %>%  # Waived trips
            mutate(random.test = ifelse(!is.na(INHERIT_TRIP_SEQ), NA, random.test)) %>% # Inherited trips aren't truly 'selected'
...
if(F){
  odds.dat %>% group_by(random.test, STRATA_CODE, TRIP_MONITOR_CODE)
...
#' `FOR 2024 ONLY`
#' *Adjusted n = 2 trips that were waived b/c of 3-in-a-row rule (verified by Geoff 3/25/24)*
#' *ODDS 3.0 not populating ODDS_STRATA_RELEASE tables with those trips so they are missing release information in data pull*

odds.dat <- 
  odds.dat %>%
  mutate(RELEASE_COMMENT = case_when(TRIP_PLAN_LOG_SEQ == 202322990 | TRIP_PLAN_LOG_SEQ == 202317623 
                                     ~ "Three Observerd Trips Release", TRUE ~ RELEASE_COMMENT),
         RELEASE_STATUS_DESCRIPTION = case_when(TRIP_PLAN_LOG_SEQ == 202322990 | TRIP_PLAN_LOG_SEQ == 202317623 
                                                ~ "Three in Row Release", TRUE ~ RELEASE_STATUS_DESCRIPTION),
         random.assign = case_when(TRIP_PLAN_LOG_SEQ == 202322990 | TRIP_PLAN_LOG_SEQ == 202317623 
                                   ~ 1, TRUE ~ random.assign),
         random.test = case_when(TRIP_PLAN_LOG_SEQ == 202322990 | TRIP_PLAN_LOG_SEQ == 202317623 
                                 ~ 0, TRUE ~ random.test),
         random.test.result = case_when(TRIP_PLAN_LOG_SEQ == 202322990 | TRIP_PLAN_LOG_SEQ == 202317623 
                                        ~ "PASS", TRUE ~ random.test.result)
         )

# How many selected (1) versus not-selected (0) trips
table(odds.dat$random.assign, useNA = "always")

#' *================================================================*
#' *Check records where trip was cancelled but selection is NA*
cancel.check <- 
  odds.dat %>%
  filter(is.na(TRIP_SELECTED), TRIP_STATUS_CODE == "CN", STRATA != "EM TRW EFP")

levels(as.factor(cancel.check$STRATA))
levels(as.factor(cancel.check$YEAR))
levels(as.factor(cancel.check$TRIP_MONITOR_CODE))
levels(as.factor(cancel.check$STRATA_CODE))
levels(as.factor(cancel.check$ODDS_SELECTION_PCT))
levels(as.factor(cancel.check$random.assign))

table(cancel.check$TRIP_SELECTED, cancel.check$TRIP_MONITOR_CODE, useNA = 'always') 
```

```{r disposition.tbl, warning=FALSE, include=FALSE}
#' *2024 AR only: split by year since we are reporting 2022 and 2023*
...
disposition <- # Merging two elements:
  merge(
    ...
    odds.dat %>%  
      select(YEAR, STRATA, random.assign, TRIP_STATUS_CODE, TRIP_PLAN_LOG_SEQ, RELEASE_STATUS_DESCRIPTION, TRIP_PLAN_NUMBER) %>%
      ...
      summarize(Trips.logged.ODDS = length(unique(TRIP_PLAN_LOG_SEQ)),
                ...
                Trips.waived= length(TRIP_PLAN_LOG_SEQ[!is.na(RELEASE_STATUS_DESCRIPTION)]),
      ...
      data.frame(),
     #' The second is the grand total *IS THIS STILL NEEDED?*
     # odds.dat %>%
...
disposition$Trips.waived[is.na(disposition$random.assign) | disposition$random.assign == 0] <- "" #' *NEW ADD*
disposition$Waiver.Pct[is.na(disposition$random.assign) | disposition$random.assign == 0] <- "" #' *NEW ADD*
...
logged <-
  ...
  filter(!ODDS_SELECTION_PCT %in% c(0, 100), !is.na(TRIP_SELECTED)) %>%  #' *LOSING ALL THOSE CN TRIPS WITH NA?*
  filter(STRATA %in% partial$STRATA & STRATA != "EM TRW EFP") %>% 
  group_by(YEAR, STRATA) %>%                                                 # 2024-ONLY: Added YEAR to group
  summarize(Total.trips = length(unique(TRIP_PLAN_LOG_SEQ)),
            ...
            Inherit.selected = length(unique(TRIP_PLAN_LOG_SEQ[random.assign == 0 & 
                                                               !is.na(INHERIT_TRIP_SEQ) &
                                                               is.na(RELEASE_STATUS_DESCRIPTION)])),
            ...
            Total.final.selected = length(unique(TRIP_PLAN_LOG_SEQ[TRIP_SELECTED == "Y"])),
...
logged.table <- bind_rows(logged, 
                      logged %>% 
                      group_by(YEAR) %>%          # 2024-ONLY: Added YEAR
                      ...
                      mutate(Pct.selected.from.inherit = format(round((Inherit.selected / Total.final.selected) * 100, 1), nsmall = 1),
                             Pct.allselected.from.waiver = format(round((Waivers.selected / (Total.final.selected + Waivers.selected)) * 100, 1), 
                                                                  nsmall = 1))) %>% 
                      select("Year" = YEAR, 
                             "Strata" = STRATA, 
...
p.table <- gather(data = for.p.wide, key, value, -STRATA, -YEAR, -Total.n, -ODDS_SELECTION_PCT, -Source)   # 2024-ONLY: Adding YEAR
...
#' Format percentages with the same number of significant figures as *(as what?)*
p.table <- p.table %>% 
           mutate(ODDS_SELECTION_PCT = round(ODDS_SELECTION_PCT, 1), 
                  pct = round(pct, 1))
...
odds.ts.daily.cuml <- odds.dat %>% # These are all logged trips
     ...
     filter(STRATA %in% partial$STRATA) %>% 
     group_by(YEAR, STRATA, Date) %>%                                # 2024 only: separate by year
     ...
     bind_rows(
          odds.dat %>% # These are the 'valid trips' only
               filter(!ODDS_SELECTION_PCT %in% c(0, 100), !is.na(TRIP_SELECTED)) %>%
               transform(Date = as.Date(PLANNED_EMBARK_DATE, format = "%Y-%m-%d")) %>% 
               filter(STRATA %in% partial$STRATA) %>% 
               group_by(YEAR, STRATA, Date) %>%                       # 2024 only: separate by year
               summarize(n = length(unique(TRIP_PLAN_LOG_SEQ[TRIP_SELECTED == "Y"])),
                         ni = length(unique(TRIP_PLAN_LOG_SEQ[!is.na(INHERIT_TRIP_SEQ)])),
                         N = length(unique(TRIP_PLAN_LOG_SEQ)),
     ...
     group_by(YEAR, STRATA, TYPE) %>%                                   # 2024 only: separate by year
     ...
     right_join(
          ...
          distinct(YEAR, STRATA, ODDS_SELECTION_PCT = Rate * 100),  # 2024 only: separate by year
          by = c("STRATA", "YEAR")) %>%  # 2024 only: separate by year
     ungroup()
...
odds.ts.daily.cuml <- transform(odds.ts.daily.cuml,
                                ...
                                YEAR = factor(YEAR, levels = c("2022", "2023"))) # 2024 only: separate by year
...
for.plot.text <- left_join(
                 ...
                 group_by(YEAR, STRATA) %>%                        # 2024 only: separate by year
                 ...
                 distinct(YEAR, STRATA, label.original, label.adjusted)   # 2024 only: separate by year
                 ,
                 partial %>% 
                 filter(STRATA != "EM TRW EFP") %>%
                 select(YEAR, STRATA, label.programmed = Rate) %>% # 2024 only: separate by year
                 mutate(STRATA = as.character(STRATA), 
                        YEAR = as.factor(YEAR),                    # 2024 only: separate by year
                        label.programmed = format(label.programmed * 100, digits = 4, nsmall = 2))
                 , by = c("STRATA", "YEAR"))                       # 2024 only: separate by year

odds.ts.daily.cuml <- merge(odds.ts.daily.cuml, for.plot.text) #' *Not needed for 2024*
...
odds.by.date.plot <- 
  ggplot(odds.ts.daily.cuml, aes(Date, running.pct, col = TYPE)) +
     ...
     facet_grid(rows = vars(STRATA), cols = vars(YEAR), scales = "free_x") +                # 2024 only: separate by year
     #' *==================For multi-year plotting==================================*
     geom_text(data = for.plot.text, aes(x = as.Date(Inf), y = 60, 
                                         label = paste("Programmed (%) = ", label.programmed, sep = "")), 
               hjust = 1, size = 3, color = "black") +
     geom_text(data = for.plot.text, aes(x = as.Date(Inf), y = 54, 
                                         label = paste("Original (%) = ", label.original, sep = "")), 
               hjust = 1, size = 3, color = "gray70") +
     geom_text(data = for.plot.text, aes(x = as.Date(Inf), y = 48, 
                                         label = paste("Final (%) = ", label.adjusted, sep = "")), 
               hjust = 1, size = 3, color = "black") +
...
#' Waiver impact *2024 only: separate by year*
waiver.min <- logged %>% 
              group_by(YEAR, STRATA) %>% 
...
pct_txt <- p.table %>% 
           ...
           mutate(pct_txt = paste0(pct, "% for the *", STRATA, "* stratum in ", YEAR)) %>%  # 2024 only: designate year
           distinct(pct_txt) 
...
# Nullify hypothesis test for any time periods with a programmed rate of 0  
...
Given the design, the level of dockside observation of walleye pollock deliveries should be 100% in the full coverage category. In `r year`, 100% of full coverage walleye pollock deliveries were observed (`r tbl_nums('Pollock.observed.table', display = "cite")`, `r tbl_nums('Pollock.efp.table', display = "cite")`).   

While expectations of the full coverage category are straightforward, evaluations of the partial coverage category are more complex. As a matter of policy, no tender deliveries are observed. While it may seem intuitive that the expected coverage rate for non-tendered deliveries within the *TRW* stratum should be equal to the programmed trip selection rate of 19.59%, this assumption is likely untrue because observers are not deployed into the pollock fishery but into the entire trawl fishery, and the relationship between the number of deliveries and trips is not expected to be constant, especially when measured across ports. Therefore, we present the dockside observation rates for non-tendered *TRW* pollock landings but make no comparison to deployment rates (`r tbl_nums('Pollock.observed.table', display = "cite")`, `r tbl_nums('Pollock.efp.table', display = "cite")`).    
...
The SSC has requested that a specific section with responses to SSC comments be provided in the written report, as is done for SAFE documents. Normally, this section would addresses FMSC responses (in italics) to comments relative to this chapter made by the Council and the SSC after the presentation of the `r year-1` Annual Report during the June `r year` Council meeting.  However, the 2019 Annual Report was not released at that meeting, and instead the deployment performance review was published separately (Ganz et al., 2020).  Therefore, there is nothing to respond to in this Chapter.   
...
# For the period when waivers were in place, remove p-values for 'After Waivers'
...
kable(Selection.table %>% 
  ...
  pack_rows(index = c("Fixed-gear EM strata : full year"=8, 
                      "Observer strata : Jan. 1 - Mar. 25"=12,
                      "Observer strata : Mar. 25 - Jun. 30"=12,
                      "Observer strata : Jul. 1 - Dec. 31"=12), indent=F) 
```
...
kbl(Coverage.Table, escape = FALSE) %>% 
  ...
  # Group rows
  pack_rows("Full coverage: Jan. 1 - Dec. 31", 1, 3, label_row_css = "text-align: center") %>% 
  pack_rows("Partial coverage EM: Jan. 1 - Dec. 31", 4, 6, label_row_css = "text-align: center") %>% 
  pack_rows("Partial coverage observed: Jan. 1 - Mar. 25", 7, 9, label_row_css = "text-align: center") %>% 
  pack_rows("Partial coverage observed: Mar. 26 - Jun. 30", 10, 12, label_row_css = "text-align: center") %>% 
  pack_rows("Partial coverage observed: Jul. 1 - Dec. 31", 13, 15, label_row_css = "text-align: center") %>%   pack_rows("Zero coverage: Jan. 1 - Dec. 31", 17, 18, label_row_css = "text-align: center")
```
