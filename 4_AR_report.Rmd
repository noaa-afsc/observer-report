...
date_cuts <- c(unique(partial_og$Effective_Date), as.POSIXct("2020-03-25"), max(odds.dat$PLANNED_EMBARK_DATE))     # 2020-ONLY
date_cuts <- date_cuts[order(date_cuts)] 

odds.dat <- mutate(odds.dat, random.assign = NA) %>% 
            mutate(PERIOD = cut(PLANNED_EMBARK_DATE, breaks=date_cuts, labels=F, ordered_result = T, right=T)) %>%  # 2020-ONLY: Adding period to odds.dat
...
kable(Selection.table %>% 
        arrange(Strata, Period, `Trip disposition`),
      col.names=c("Strata", "Period", "Trip disposition", "Selected trips", "Total trips", "Actual selection (%)", "Programmed selection (%)", "*p*-value (H0: Actual = Programmed)"), escape=F) %>%
  kable_paper()

# Option for wide version
kable(bind_cols(Selection.table %>% 
            filter(Period==1) %>%
            select(-Period) ,
          Selection.table %>%
            filter(Period==2) %>%
            select(-Strata, -`Trip disposition`, -Period),
          Selection.table %>%
            filter(Period==3) %>% 
            select(-Strata, -`Trip disposition`, -Period)),
    align="llrrrrrrrrrrrrrrr",
    col.names=c("Strata", "Trip disposition", rep(c("*n*", "*N*", "Actu", "Prog", "*p*"), times=3)), escape=F) %>%
  kable_paper() %>%
  collapse_rows(columns=1) %>%
  add_header_above(c(" "=2, "Jan1 - Mar24" = 5,  "Mar24 - Jun30" = 5, "July1 - Dec31" = 5))
```
