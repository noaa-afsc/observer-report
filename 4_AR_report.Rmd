...
#' was less than the trip's selection rate. Since ODDS 3.0 was online in 2023, all trips have `RANDOM_NUMBER_USED` 
...
# Redefine values in the random.assign
...
#' Summary of ODDS [completed trips] disposition by stratum, with total completed trips, subdivided by mutually 
#' exclusive categories of randomly selected monitored trips, inherited monitored trips, released randomly selected 
#' trips, and released inherited trips.
...
logged <- odds.dat %>% 
    ...
    summarize(
      ...
      Waivers.inherited = length(TRIP_PLAN_LOG_SEQ[!is.na(INHERIT_TRIP_SEQ) & !is.na(RELEASE_STATUS_DESCRIPTION)]),                          # FIXME Could be different if the same trip was inherited multiple times?
...
for.p.wide <- merge(
  # Completed non-100% monitored trips only
  logged %>% 
    #' *2023 AR ONLY: Added YEAR to group*
    group_by(YEAR, STRATA) %>%
    summarize(random.n = Random.selected,
              inherit.n = Total.final.selected,
              inherit.butnowaiver.n = (Total.final.selected + Waivers.selected + Waivers.inherited),            #' *total before waivers*
              Total.n = Total.trips,
...
#' TODO *FIXME* These 'final' rates should coincide with our Table 3-4 (a-b-c-d) - I'm not sure how in the 2019 AR that
#' these rates matched up when the logic below is not accounting for waived trips at all! It used to filter on
#' TRIP_SELECTED, which may have not only removed cancelled trips but also waived trips??

odds.ts.daily.cuml <- odds.dat %>% 
     # These are all logged trips
     ...
     ungroup() %>% 
     bind_rows(
          # These are the 'valid trips', i.e., completed trips, only
          odds.dat %>% 
               filter(!ODDS_SELECTION_PCT %in% c(0, 100), !(TRIP_STATUS_CODE %in% c("CN", "CC", "CS", "CR"))) %>%  #'* Before had !is.na(TRIP_SELECTED_OBS), which removed waived trips? *
               transform(Date = as.Date(PLANNED_EMBARK_DATE, format = "%Y-%m-%d")) %>% 
               filter(STRATA %in% partial$STRATA) %>% 
               #' *2023 AR only: separate by year*
               group_by(YEAR, STRATA, Date) %>%
               summarize(n = length(unique(TRIP_PLAN_LOG_SEQ[TRIP_SELECTED == "Y"])),          #' *FIXME reference to trip_selected*
                         ni = length(unique(TRIP_PLAN_LOG_SEQ[!is.na(INHERIT_TRIP_SEQ)])),
                         N = length(unique(TRIP_PLAN_LOG_SEQ)),
     ...
     mutate(running.n = order_by(Date, cumsum(n)),                                           # cumulative totals of n an N by type
            running.N = order_by(Date, cumsum(N)), 
            rug_ni = if_else(ni != 0, Date, as.Date(NA)),                                    # if there is an inherited trip, copy the date, otherwise NA
            running.pct = round((running.n / running.N) * 100, 2)) %>%     # TODO So is this test ONLY looking at RN before and after cancellations? waivers excluded?
     arrange(STRATA, TYPE, Date) %>%                                       # Then why are the rates in 2019 Fig 3-2 matching Table 3-4 (d) after waivers?
     right_join(
          partial %>%
...
kable(
  ...
  col.names=c("Year" , "Strata", "Trip disposition", "Selected<br>trips", "Total<br>trips", "Actual<br>(%)", "Programmed<br>(%)", "*p*-value"), 
  escape=F, align="lllrrrrr"
) %>% kable_paper() 
