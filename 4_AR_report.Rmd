...
logged.table <- bind_rows(logged, 
                      ...
                      mutate(Pct.selected.from.inherit = format(round((Inherit.selected / Total.final.selected) * 100, 1), nsmall = 1),
                             Pct.allselected.from.waiver = format(round((Waivers.selected / (Total.final.selected + Waivers.selected)) * 100, 1), nsmall = 1))) %>% 
                      select(`Year` = YEAR, `Strata` = STRATA, `Total Trips` = Total.trips,
...
#for.p.wide_TEST <- merge(
#  for.p.wide, 
#  merge(partial %>%                             
#        filter(STRATA != "EM TRW EFP") %>%
#        distinct(STRATA) %>%
#        crossing(data.frame(YEAR = 1:2)),
#      partial %>% filter(STRATA != "EM TRW EFP") %>% 
#        distinct(STRATA, ODDS_SELECTION_PCT = Rate * 100) %>%
#        group_by(STRATA) %>%
#        mutate(YEAR = seq_len(n()))  %>%
#        mutate(YEAR = ifelse(YEAR == 1, YEAR + 1, YEAR)), all = T
#      )  %>% 
#  group_by(STRATA) %>%
#  mutate(ODDS_SELECTION_PCT = ifelse(YEAR == 2, ODDS_SELECTION_PCT[YEAR == 1], 
#                                     ifelse(STRATA %like% "EM", ODDS_SELECTION_PCT[YEAR == 1], 
#                                            ifelse(is.na(ODDS_SELECTION_PCT), ODDS_SELECTION_PCT[YEAR == 1], ODDS_SELECTION_PCT)))), 
#  by = c("STRATA", "YEAR"), all = TRUE)
