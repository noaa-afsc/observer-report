...
\
Proximity indices were calculated for each stratum to evaluate whether coverage met expectations overall (`r fig_nums('interspersion', display = "cite")`), and for the BSAI and GOA separately following the use of Fishery Management Plan (FMP) in the stratum definition for the `r year` ADP (NMFS 2023b, `r fig_nums('interspersion_fmp', display = "cite")`).\
\
The spatiotemporal distributions of monitoring coverage in strata were within the expected ranges (the upper, green distributions of Figures `r fig_nums('interspersion', display = "num")` and `r fig_nums('interspersion_fmp', display = "num")`) with only one exception. The proximity value of the EM HAL stratum was lower than expected (`r fig_nums('interspersion', display = "cite")`), particularly in the GOA (`r fig_nums('interspersion_fmp', display = "cite")`). Therefore, a spatiotemporal bias was apparent in the EM HAL stratum based on the available data. The proximity value for the EM TRW EFP stratum was low but not unexpected, and resulting proximity values were very close to 1 (indicating good spatiotemporal overlap).\
\
The achieved proximity values were within the expected ranges from the sampling rates defined by the ADP (the lower, blue distributions of Figures `r fig_nums('interspersion', display = "num")` and `r fig_nums('interspersion_fmp', display = "num")`) for the *OB HAL*, *OB POT* and *EM TRW EFP* strata. The *OB TRW* stratum had a higher proximity index than expected because it had an unexpectedly high realized monitoring rate. Both fixed-gear EM strata, *EM HAL* and *EM POT*, had lower proximity indices than expected due to the lower than expected realized monitoring rates. At the FMP-level, proximity values for these strata were lower than expected except for in the BSAI of the *EM HAL* stratum, which had a very wide expected range due to low fishing effort and low expected sample size (`r fig_nums('interspersion_fmp', display = "cite")`).
