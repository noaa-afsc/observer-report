...
load("G:\\FMGROUP\\Observer Program Annual Report\\2021_Annual_Report\\2_AR_data.RData") # This includes the .Random.seed set in 1_AR_data.R
...
partial_time <- partial %>% 
                ...
                mutate(Effective_Date = as.Date(Effective_Date), Expire_After = as.Date(c("2021-12-31", "2021-12-31", "2021-12-31", "2021-08-31", "2021-08-31", "2021-08-31"))) %>% 
...
partial_time_hardcoded <- partial_time %>% 
  mutate(Rate = ifelse(STRATA == 'HAL', 0.1513, Rate)) %>%
  mutate(Rate = ifelse(STRATA == 'POT', 0.1504, Rate)) %>% 
  mutate(Rate = ifelse(STRATA == 'TRW', 0.1612, Rate))
...
```{r map_generation, message=FALSE, warning=FALSE, eval=FALSE, echo = FALSE}
# Map generation
...
```{r strata_patterns_by_stratum_text, message=FALSE, warning=FALSE, eval=FALSE, echo = FALSE}
...
kbl(Coverage.Table, escape = FALSE) %>% 
  ...
  pack_rows("Partial coverage observed: Jan. 1 - Aug. 31", 7, 9, label_row_css = "text-align: center") %>% 
  pack_rows("Partial coverage observed: Sep. 1 - Dec. 31", 10, 12, label_row_css = "text-align: center") %>% 
  #pack_rows("Partial coverage observed: Sep. 1 - Dec. 31", 13, 15, label_row_css = "text-align: center") %>%   
  pack_rows("Zero coverage: Jan. 1 - Dec. 31", 14, 15, label_row_css = "text-align: center")
```
...
grid.arrange(outcomes.plot, costs.plot, nrow=2)
```
