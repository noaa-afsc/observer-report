...
OBSERVED_FLAG.results <- rbind(
  ...
  OBSERVED_FLAG.results %>% 
    ...
    group_by(Strata, Observed, Unobserved) %>% 
...
Obs.perm.plot <- ggplot(OBSERVED_FLAG.YN_perm_plot, aes(x = perm_result_pct)) +
  ...
  geom_label(data = OBSERVED_FLAG.summary_plot, 
             aes(label = paste("italic(p) == ", round(p,3), sep = "")),
             x=-Inf, y=Inf, hjust=-.2, vjust=1.2, size = 4, parse=TRUE) +
  fig.theme +
...
# TODO - MOVE TRIP COUNTS TO TABLE 10 BELOW, REMOVE THIS TABLE
