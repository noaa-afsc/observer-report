...
The cumulative number of fishing trips in each stratum was multiplied by the stratum-specific selection rate to obtain the expected number of observed trips. Under the assumption that there is no temporal bias in observer coverage, 2.5% of values should be larger than the upper 95% confidence limit and 2.5% should be smaller than the lower limit. At the end of `r year` the number of observed trips was outside of this expected range in one of the seven partial coverage strata: *EM FIXED BSAI* (expected rate = 74.29%, realized rate = 49.28%, *p*-value < 0.001); `r tbl_nums('Coverage.Table', display = "cite")` and `r fig_nums('Temporal.plot', display = "cite")`). The *EM FIXED BSAI* stratum was below the expected range for a brief period in July and then for the remainder of the year beginning in August [**CHECK THESE MONTHS**]. Coverage rates were within their expected ranges for 100% of the year for most other strata, with the exception of *OB FIXED GOA* and *EM FIXED GOA* which were very briefly above the expected range early in the year.
