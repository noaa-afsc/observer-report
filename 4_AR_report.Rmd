...
load("source_data/2_AR_data.RData") # This includes the .Random.seed set in 1_AR_data.R
...
# Set order of strata
strata.order <- c("Full", "OB HAL", "OB POT", "OB TRW", "EM HAL", "EM POT", "EM TRW EFP", "Zero coverage", "Total")
...
* **Deploy for the planned number of sea days.** This objective will be considered to be met if the actual number of sea days expended falls within the range of values from simulated sampling provided in the `r year` ADP. The Observer Program’s budget was expected to cover `r formatC(AvailableYR.2023, format = "d", big.mark = ",")` days in `r year`.\
* **Deploy at the coverage rates specified in the `r year` ADP.** Following the `r year` ADP, ODDS was programmed to randomly select logged trips at a rate of `r formatC(partial$Rate[partial$STRATA == "OB TRW" & partial$YEAR == year] * 100, format = "f", digits = 2)`% in the *OB TRW* stratum, `r formatC(partial$Rate[partial$STRATA == "OB HAL" & partial$YEAR == year] * 100, format = "f", digits = 2)`% in the *OB HAL* stratum, `r formatC(partial$Rate[partial$STRATA == "OB POT" & partial$YEAR == year] * 100, format = "f", digits = 2)`% in the *OB POT* stratum, and `r formatC(partial$Rate[partial$STRATA == "EM POT" & partial$YEAR == year] * 100, format = "f", digits = 2)`% in the EM strata. Under a randomized deployment scheme, these partial coverage selection rates are expected to be within a 95% confidence interval computed from the realized coverage rates (under the assumption of a binomial distribution for observed trips).\
...
The methods used in this analysis are similar to those employed in the gap analysis in Appendix C of the 2020 Draft Annual Deployment Plan and Appendix B of the 2019 Annual Report and are published in Ganz et al. (2020). Partial coverage fishing effort data from 2020 were used to quantify the degree to which data from monitored trips are available within specified spatiotemporal distances to unmonitored fishing trips. Prior versions of this analysis had quantified the degree of overlap in terms of an index. Here, we only use presence and absence of fishing effort and monitored fishing effort in each week, each NMFS area, and each stratum. More detailed versions of these plots with species target are planned to be available to stock assessment authors separate from this document.**
...
outcomes.plot.2023 <- ggplot(bud_tbl.2023, aes(x = ADP_D)) +
  ...
  theme(text = element_text(size = 14),
        ...
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) +
  scale_x_continuous(limits = c(2450, 3750), breaks = seq(2000, 3500, 500)) +
...
#' *Table 3-1 : Comparison between predicted and actual trip days for partial coverage strata in 2022 and 2023.*
...
In `r year`, the FMA paid for `r formatC(round(ActualYR.2023), format = "d", big.mark = ",")` observer days, which was `r ifelse(eval.2023$pct.budget.remain != 0, abs(eval.2023$pct.budget.remain), " ")` `r ifelse(eval.2023$pct.budget.remain != 0, "%", "")` `r eval.2023$more.or.less` predicted by the average simulation, but well within the range of possibilities predicted in the `r year` ADP (`r fig_nums('outcomes.plot.2023', display = "cite")`, top panel). Expenditures for partial observer coverage were under budget (`r fig_nums('outcomes.plot.2023', display = "cite")`, bottom panel). Lower expenditures resulted because there were fewer observer days than predicted and the cost of a partial coverage observer day in `r year` was less than the expected cost that was estimated in the `r year` ADP.     
...
#odds.by.date.plot <- 
 # ggplot(odds.ts.daily.cuml, aes(x = Date, col = fct_rev(TYPE))) +
  ...
  #xlab("\nDate") +
  #geom_rug(data = odds.ts.ni.create %>% filter(ni.create > 0), sides = "b", color = "gray50") + 
  #geom_rug(data = odds.ts.ni.real %>% filter(ni.real > 0), sides = "t") + 
  #' *==================For multi-year plotting==================================*
  #facet_grid(factor(STRATA, levels = strata.order) ~ YEAR, scales = "free") +
...
odds.by.date.plot.2022 <- 
  ggplot(filter(odds.ts.daily.cuml, YEAR == year - 1), aes(x = Date, col = fct_rev(TYPE))) +
  # Add lines for selection rates  
  geom_hline(aes(yintercept = ODDS_SELECTION_PCT), lty = 3, linewidth = 1) +
  geom_ribbon(aes(ymin = L95 * 100, ymax = H95 * 100), alpha = 0.1) +
  geom_line(aes(y = running.pct), linewidth = 1.5) +
  ...
  #coord_cartesian(ylim = c(0, 70)) +
  xlab("\nDate") +
  geom_rug(data = odds.ts.ni.create %>% filter(ni.create > 0 & YEAR == year - 1), sides = "b", color = "gray50") + 
  geom_rug(data = odds.ts.ni.real %>% filter(ni.real > 0 & YEAR == year - 1), sides = "t") + 
...
perc.syst.canc.2022 <- disposition %>% 
                       filter(STRATA == "Total" & random.assign == "Selected", YEAR == "2022") %>% 
                       summarize(perc.syst.canc = round((sum(as.numeric(Trips.canceled.bysystem)) / sum(as.numeric(Trips.logged.ODDS))) * 100, 1)) %>% 
                       as.numeric()
...
perc.user.canc.2022 <- disposition %>% 
                       filter(STRATA == "Total" & random.assign == "Selected", YEAR == "2022") %>% 
                       summarize(perc.syst.canc = format((sum(as.numeric(Trips.canceled.byusers)) / sum(as.numeric(Trips.logged.ODDS))) * 100,
                                                         digits = 1, nsmall = 1)) %>% 
                       as.character()
...
Logged trips have different dispositions. When initially logged, trips are considered pending, and subsequently have two dispositions: closed or canceled. A trip can be closed by (1) selecting landing reports from a menu or (2) manually entering the end of trip information for observed trips. The vessel operator may change the dates of a logged trip regardless of selection status prior to, or in lieu of, cancellation. However, trips that have not been closed at the end of the calendar year are automatically canceled by the ODDS to prevent `r year` ODDS trips from affecting the deployment rates set for the `r year + 1` ADP. Trips that were selected to be monitored by ODDS and are subsequently canceled trigger the next logged trip to automatically inherit the selected status. These trips are termed inherited trips.  

The number of trips logged in the ODDS in `r year` and their dispositions is summarized in `r tbl_nums('Selection.table.2023', display = "cite")`. Of the `r formatC(sum(disposition$Trips.logged.ODDS[disposition$STRATA == "Total" & disposition$YEAR == 2023]), format = "d", big.mark = ",")` total trips logged, `r formatC(disposition$Trips.logged.ODDS[disposition$STRATA == "Total" & disposition$random.assign == "Selected" & disposition$YEAR == "2023"], format = "d")` were selected, and `r formatC(totl.canc.2023, format = "d", big.mark = ",")` were canceled: `r formatC(disposition$Trips.canceled.bysystem[disposition$STRATA == "Total" & disposition$random.assign == "Selected" & disposition$YEAR == "2023"], format = "d", big.mark = ",")` by ODDS (`r perc.syst.canc.2023`%) and `r formatC(disposition$Trips.canceled.byuser[disposition$STRATA == "Total" & disposition$random.assign == "Selected" & disposition$YEAR == "2023"], format = "d", big.mark = ",")` by users (`r perc.user.canc.2023`%). The user cancellation rate for selected trips ranged from `r sel.min.canc$Cancellation.Pct[sel.min.canc$YEAR == 2023]`% for *`r sel.min.canc$STRATA[sel.min.canc$YEAR == 2023]`* to `r sel.max.canc$Cancellation.Pct[sel.max.canc$YEAR == 2023]`% for *`r sel.max.canc$STRATA[sel.max.canc$YEAR == 2023]`* in `r year`.          
...
 The `r year` Observer Program had `r total.strat` different deployment strata to be evaluated (`r tbl_nums('Coverage.Table.2023', display = "cite")`). There were two *Full* coverage strata comprised of trips taken both by vessels that were required to have full coverage (e.g., AFA vessels) and those fishing in the BSAI that opted into full coverage (i.e., *EM TRW EFP*). There were six partial coverage strata: three observed strata defined by gear designation and three EM strata defined by gear designation. There was also one *Zero Coverage* stratum for jig vessels and vessels under 40 ft. length overall.      
...
Evaluations for the full coverage category and zero-selection pool are straightforward --- either the coverage achieved was equal to 100% or 0%, respectively, or it was not. The program achieved 99.7% coverage in its full coverage category (`r tbl_nums('Coverage.Table.2023', display = "cite")`). Four trips were not monitored in the full coverage category --- **UPDATE four of these occurred on a single catcher vessel fishing CDQ halibut with hook and line gear that retained Pacific cod above the Maximum Retainable Amount and therefore met the criteria for full coverage fishing but failed to obtain a full coverage observer.** The program achieved perfect compliance with the *Zero Coverage* stratum (`r tbl_nums('Coverage.Table.2023', display = "cite")`). Under the assumption that the deployment was randomized, a 95% confidence interval computed from the realized coverage rates (under the assumption of a binomial distribution for observed trips) will contain the actual deployment rate 95% percent of the time. If expected coverage levels were within the 95% confidence intervals, then we conclude that realized and expected coverage rates were equal. Coverage rates were consistent with expected values in only half of the six partial coverage strata. Coverage rates were lower than expected for *EM HAL* and *EM POT*, but were higher than expected within the *OB TRW* stratum (`r tbl_nums('Coverage.Table.2023', display = "cite")`). **UPDATE There are two reasons why this result is of little concern. First, there is no clear evidence of trip manipulation in ODDS data from this stratum. Secondly, the achieved rate was only slightly outside of the 95% range of expected outcomes (16.1% achieved vs. a lower bound of 16.8%). Given the low number of total trips in this stratum (44), a change in a single observed trip, from 13 observed to 12 would have resulted in an expected result for this stratum since new confidence bounds would have included the expected rate.**

Unlike observed trips, the coverage rate for EM is based on information provided from the Pacific States Marine Fisheries Commission (PSMFC) that is available to analysts in the NORPAC database. In `r year`, the median time between receipt and completion of review was **UPDATE** days for *EM HAL* and **UPDATE** days for *EM POT* (`r fig_nums('em.times.figure', display = "cite")`). This is compared to a median of seven days during pre-implementation in 2016 (NMFS 2017a, p. 87).

In combination across all strata, coverage levels, and fishery monitoring tools, `r formatC(as.numeric(Coverage.Table$n[Coverage.Table$Year == 2023 & Coverage.Table$Strata == "Total"]), format = "d", big.mark = ",")` trips (43.7%) and `r formatC(Coverage.Table$v[Coverage.Table$Year == 2023 & Coverage.Table$Strata == "Total"], format = "d", big.mark = ",")` vessels (50.2%) were successfully monitored at-sea among all fishing in Federal fisheries of Alaska in `r year` (`r tbl_nums('Coverage.Table.2023', display = "cite")`).  

### Coverage Rates for Dockside Monitoring
 Observers were assigned to monitor shoreside deliveries of pollock. The objective of this monitoring was to obtain a count of the number of salmon caught as bycatch and to obtain tissue samples for genetic analysis from these fish in each observed pollock delivery. The sampling design used for this objective in `r year` remained unchanged from that used since 2011 (Faunce 2015); all deliveries of pollock that were observed at sea were also observed dockside. While all Bering Sea pollock trips and deliveries are observed, this is not the case in the GOA (NMFS 2015), where pollock trips randomly selected for at-sea monitoring are also expected to be sampled shoreside for salmon. For this analysis, pollock deliveries are defined as any delivery where the predominant species is pollock in eLandings.     
...
# table(Pollock.landing.data$COVERAGE_TYPE, Pollock.landing.data$Observed.type, Pollock.landing.data$TENDER, Pollock.landing.data$ADP)
...
Given the design, the level of dockside observation of pollock deliveries should be 100% in the full coverage categories. In `r year`, 100% of full coverage pollock deliveries were observed (`r tbl_nums('Pollock.observed.table', display = "cite")`, `r tbl_nums('Pollock.efp.table', display = "cite")`).

While expectations of the full coverage category are straightforward, evaluations of the partial coverage category are more complex. As a matter of policy, no tender deliveries are observed. While it may seem intuitive that the expected coverage rate for non-tendered deliveries within the *OB TRW* stratum should be equal to the programmed trip selection rate of `r formatC(partial$Rate[partial$YEAR == 2023 & partial$STRATA == "OB TRW"] * 100, format = "f", digits = 2)`%, this assumption is likely untrue because observers are not deployed into the pollock fishery but into the entire trawl fishery, and the relationship between the number of deliveries and trips is not expected to be constant, especially when measured across ports. Therefore, we present the dockside observation rates for non-tendered *OB TRW* pollock landings but make no comparison to deployment rates (`r tbl_nums('Pollock.observed.table', display = "cite")`, `r tbl_nums('Pollock.efp.table', display = "cite")`).
...
For.ts.temporal.plot <- left_join(For.ts.temporal.plot,
                              select(partial, c(YEAR, STRATA, Rate)), by = c("YEAR", "STRATA"))
...
For.ts.temporal.summary <- 
  ...
  reframe(N.outside = sum(Outside.bounds, na.rm = TRUE),
          Days = ifelse(YEAR == 2022,
                        sum(Date >= as.Date("2022-01-01")),
                        sum(Date >= as.Date("2023-01-01")))) %>%
  distinct() %>% 
...
For.ts.temporal.plot <- transform(For.ts.temporal.plot,
                                  STRATA = factor(STRATA,levels = c("EM HAL", "EM POT", "OB HAL", "OB POT", "OB TRW")))
...
# Multi-Year Plot
#Temporal.plot <- 
 # ggplot(For.ts.temporal.plot, aes(x = Date)) +
  ...
  #facet_grid(factor(STRATA, levels = strata.order) ~ YEAR, scales = "free", space = "free_x") +
    # Annotate p-values
  #geom_text(aes(x = as.Date(-Inf), y = Inf, vjust = 1.25, label = p), 
  #          hjust = -0.1, size = 4, parse = TRUE, check_overlap = TRUE) +
  # Plot invisible points to ensure x-facets span the entire year
  #geom_point(data = Temporal.plot.x_range, y = 0, alpha = 0) +
  #theme_bw() +
  #theme(axis.text.x = element_text(angle = 90)) +
...
# Single-Year Plot
Temporal.plot.2022 <- 
  ggplot(filter(For.ts.temporal.plot, YEAR == year - 1), aes(x = Date)) +
  # Add lines for coverage rates
  geom_line(aes(y = n.cuml), col = "black", linewidth = 2) +
  geom_line(aes(y = n.expected), col = "gray") +
  geom_ribbon(aes(ymin = Low95, ymax = High95), alpha = 0.2) +
  ...
  ylab("Cumulative trips in trip-selection strata\n") +
  xlab("\nDate") +
  scale_x_date(limits = as.Date(c("2022-01-01", "2023-01-01")))
...
```{r cell.pot, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE}
...
```{r map_generation, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE}
# Map generation
...
```{r spatial_patterns_by_stratum, message=FALSE, warning=FALSE, include=FALSE, results="asis", eval=FALSE}
...
```{r strata_patterns_by_stratum_text, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE}
...
The sample sizes available and the results of permutation tests are presented in <!--`r tbl_nums('OBSERVED_FLAG.results', display = "cite")`-->.  A visual depiction of individual results of this permutation test for the *HAL*, *POT - No Tender*, and *TRW - No Tender* strata is given in <!--`r fig_nums('Obs.perm.plot', display="cite")`--> for illustration purposes.     
 
```{r observed_flag_permutation, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE}
...
```{r strata_observed_flag_text, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE}
...
```{r suppress, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE}
<!-- Now knit and paste 'out' to create the paragraphs of text. -->
`r paste(knit(text = out), collapse = '\n')`
```
  
```{r observed_permutation_plot, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE}
...
``` {r freq_geartender, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE} 
...
``` {r prob.bynumber.plot, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE}
...
```{r trips_by_tolerance, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE}
...
**2. The sampling design for the next ADP should use trip as the primary sampling unit and should not be constrained by port of departure or landing unless doing so jeopardizes health and safety.  We support a rollover of the final 2020 ADP as originally proposed for 2022.** 
...
####`r tbl_nums('trip.budget.tbl.2022', caption = paste0('Comparison between predicted and actual trip days for partial coverage strata in ', year - 1,'.  Predicted values come from the ', year - 1,' Annual Deployment Plan (ADP).'))`  

```{r trip.budget.tbl.2022, echo=FALSE, tab.cap=""}
...
flextable(trip.budget.table.2022) %>%
  ...
  hline(i = c(3, 6, nrow(trip.budget.table.2022)), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  add_header_row(values = c("", "Trip days", "Difference"), colwidths = c(1, 2, 2)) %>%
  colformat_num(j = 3, i = c(2, 4:7), digits = 0) %>% # This year has half day realized increments for actual trip days
...
####`r tbl_nums('trip.budget.tbl.2023', caption = paste0('Comparison between predicted and actual trip days for partial coverage strata in ', year,'.  Predicted values come from the ', year,' Annual Deployment Plan (ADP).'))`  

```{r trip.budget.tbl.2023, echo=FALSE, tab.cap=""}

trip.budget.table.2023 <- trip.budget.table.2023 %>%
  mutate(pred_days = as.numeric(pred_days),
         act_days = as.numeric(act_days)) %>%
  arrange(factor(STRATA, levels = strata.order))
...
flextable(trip.budget.table.2023) %>%
  set_header_labels(values = list(
    STRATA = "Strata",
    pred_days = "Predicted",
    act_days = "Actual",
  ...
  italic(j = 1, i = 1:6) %>%
  bold(bold = TRUE, part = "header") %>%
  hline(i = c(3, 6, nrow(trip.budget.table.2023)), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  add_header_row(values = c("", "Trip days", "Difference"), colwidths = c(1, 2, 2)) %>%
  width(j = 1, 1.125, unit = "in") %>%
  width(j = 2, 0.8, unit = "in")
...
####`r tbl_nums('disposition.table.2022' , caption = paste0('Trip cancellation rates in the ODDS for ', year - 1, '. A trip is canceled by the system if the user did not identify whether fishing had occurred by the end of the year. “Paper” indicates that a trip was logged when the ODDS was not available.'))` 
  
```{r disposition.table.2022, echo=FALSE, tab.cap=""}
# Table 2 from 2019 Annual Report
#' *NOT INCLUDED SINCE 2019*
...
flextable(disposition.table.2022) %>%
  ...
  bold(bold = TRUE, part = "header") %>%
  merge_v(j = 1) %>%
  hline(i = seq(2, nrow(disposition.table.2022), 2)) %>%
  hline(i = c(6, 10, nrow(disposition.table.2022)), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  fix_border_issues() %>%
  width(j = 1, 0.7, unit = "in") %>%
  width(j = c(2, 9), 0.94, unit = "in") %>%
  width(j = c(3, 10), 0.63, unit = "in") %>%
  width(j = 4:6, 0.81, unit = "in") %>%
...
####`r tbl_nums('disposition.table.2023' , caption = paste0('Trip cancellation rates in the ODDS for ', year, '. A trip is canceled by the system if the user did not identify whether fishing had occurred by the end of the year. “Paper” indicates that a trip was logged when the ODDS was not available.'))` 
  
```{r disposition.table.2023, echo=FALSE, tab.cap=""}
# Table 2 from 2019 Annual Report
#' *NOT INCLUDED SINCE 2019*

# 2023

disposition.table.2023 <- disposition.table %>%
  filter(Year == 2023) %>%
  select(-Year)

flextable(disposition.table.2023) %>%
  mk_par(j = 3, part = "header",
         value = as_paragraph("Logged (", as_i("a"), ")")) %>%
  mk_par(j = 4, part = "header",
         value = as_paragraph("Canceled by system (", as_i("b"), ")")) %>%
  ...
  hline(i = c(6, 10, nrow(disposition.table.2023)), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  fix_border_issues() %>%
  width(j = 1, 0.7, unit = "in") %>%
  width(j = c(2, 9), 0.94, unit = "in") %>%
...
####`r tbl_nums('logged.table.2022', caption = paste0('Number of remaining trips after cancellation in each trip-selection stratum that were selected using the initial random number generator (Random Number Selection) and those that remained after user manipulation (Total Final Selected) in ', year - 1, '. The relative impact of waivers in trip-selection is also shown (% Reduction of Selected Trips due to Waivers). **Not from random numbers.'))` 

```{r logged.table.2022, echo=FALSE, tab.cap=""}
...
flextable(logged.table.2022) %>%
  ...
  bold(bold = TRUE, part = "header") %>%
  merge_v(j = 1) %>%
  hline(i = c(3, 5, nrow(logged.table.2022)), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  fix_border_issues() %>%
  width(j = 1, 0.69, unit = "in") %>%
  width(j = 2, 0.5, unit = "in") %>%
  width(j = 3:4, 0.75, unit = "in") %>%
  width(j = 5, 0.66, unit = "in") %>%
  width(j = 6, 1.25, unit = "in") %>%
  width(j = 7, 0.94, unit = "in") %>%
  width(j = 8, 1.81, unit = "in")
...
```{r logged.table.2023, echo=FALSE, tab.cap=""}
# Table 3 from 2019 Annual Report
#' *NOT INCLUDED SINCE 2019*

# 2023

logged.table.2023 <- logged.table %>%
  filter(Year == 2023) %>%
  select(-Year)

flextable(logged.table.2023) %>%
  mk_par(j = 3, part = "header",
         value = as_paragraph("Random number selection (", as_i("r"), ")")) %>%
  mk_par(j = 4, part = "header",
         value = as_paragraph("Inherited selection **(", as_i("i"), ")")) %>%
  ...
  bold(bold = TRUE, part = "header") %>%
  merge_v(j = 1) %>%
  hline(i = c(3, 5, nrow(logged.table.2023)), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  fix_border_issues() %>%
  width(j = 1, 0.69, unit = "in") %>%
  width(j = 2, 0.5, unit = "in") %>%
...
flextable(Selection.table.2022) %>%
  ...
  mk_par(j = 2, i = seq(1, nrow(Selection.table.2022), 4), part = "body",
         value = as_paragraph("Initial random selection, ", as_i("a"))) %>%
  mk_par(j = 2, i = seq(2, nrow(Selection.table.2022), 4), part = "body",
         value = as_paragraph("After cancellations, ", as_i("b"), " (", as_i("a"), " - ", as_i("b"), ")")) %>%
  mk_par(j = 2, i = seq(3, nrow(Selection.table.2022), 4), part = "body",
         value = as_paragraph("With inherits, ", as_i("c"), " (", as_i("a"), " - ", as_i("b"), " + ", as_i("c"), ")")) %>%
  mk_par(j = 2, i = seq(4, nrow(Selection.table.2022), 4), part = "body",
  ...
  italic(j = 1) %>%
  merge_v(j = 1) %>%
  hline(i = seq(4, nrow(Selection.table.2022), 4)) %>%
  hline(i = c(12, nrow(Selection.table.2022)), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  fix_border_issues() %>%
  width(j = c(1, 3), 0.69, unit = "in") %>%
  width(j = 2, 2.21, unit = "in") %>%
  width(j = c(4, 5, 7), 0.63, unit = "in") %>%
  width(j = 6, 1.06, unit = "in")

```

#####

####`r tbl_nums('Selection.table.2023', caption = paste0('Number of logged trips in each partial coverage stratum in ', year, ' that were selected using the initial random number generator (Initial Random Selection) and those that remained after user manipulation (After Cancellations). The relative impact of inherits and waivers in trip-selection is also shown (With Inherits, After Waivers). Note that observer strata were split into three separate time periods to reflect when waivers were put in place and when ODDS selection rates were adjusted to account for changes to the sample frame from port-based trip deployment'))`  

```{r Selection.table.2023, echo=FALSE, tab.cap=""}

# 2023

Selection.table.2023 <- Selection.table %>%
  filter(Year == 2023) %>%
  select(-Year)

flextable(Selection.table.2023) %>%
  set_header_labels(values = list(
    n = "Selected trips",
    N = "Total trips",
    Actu = "Actual (%)",
  ...
  hline(i = c(12, nrow(Selection.table.2023)), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  fix_border_issues() %>%
  width(j = c(1, 3), 0.69, unit = "in") %>%
  width(j = 2, 2.21, unit = "in") %>%
...
####`r tbl_nums('Coverage.Table.2022', caption = paste0('Number of total vessels (*V*), sampled vessels (*v*), total trips (*N*), sampled trips (*n*) for each stratum in ', year - 1, '. The coverage and 95% confidence interval columns are expressed as percentages of the total number of trips taken within each stratum.'))`  

```{r Coverage.Table.2022, echo=FALSE, tab.cap=""}
...
as_grouped_data(Coverage.Table.2022, groups = "Coverage") %>%
  ...
  align(part = "all", align = "center") %>%
  align(i = ~ !is.na(Coverage), align = "left") %>%
  align(j = 10, align = "left") %>%
  align(j = 1, i = 15, align = "left") %>%
  bold(bold = TRUE, part = "header") %>%
  ...
  hline(i = c(3, 8, 11)) %>%
  hline(i = 14, border = officer::fp_border(width = 2)) %>%
  surround(i = ~ !is.na(Coverage), border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  ...
  padding(j = 7, i = 15, part = "body", padding.left = 20)

```

#####

####`r tbl_nums('Coverage.Table.2023', caption = paste0('Number of total vessels (*V*), sampled vessels (*v*), total trips (*N*), sampled trips (*n*) for each stratum in ', year, '. The coverage and 95% confidence interval columns are expressed as percentages of the total number of trips taken within each stratum.'))`  

```{r Coverage.Table.2023, echo=FALSE, tab.cap=""}
  
# 2023
Coverage.Table.2023 <- Coverage.Table %>%
  filter(Year == 2023) %>%
  select(-Year)

as_grouped_data(Coverage.Table.2023, groups = "Coverage") %>%
  as_flextable(hide_grouplabel = TRUE) %>%
  align(part = "all", align = "center") %>%
  align(i = ~ !is.na(Coverage), align = "left") %>%
  align(j = 10, align = "left") %>%
...
```{r em.summary.table, echo=FALSE, tab.cap="", eval=F, include=F, eval=FALSE}
...
####`r tbl_nums('Pollock.observed.table.2022', caption = paste0('The number of pollock deliveries made by vessels in the *OB TRW* stratum during ', year - 1, ', separated by port and coverage category. Trips that delivered to a tender have been excluded. Observed deliveries denote deliveries that were observed shoreside for salmon.'))`  

```{r Pollock.observed.table.2022, echo=FALSE, tab.cap=""}
...
#flextable(Pollock.observed.table) %>%
  ...
  #align(part = "all", align = "center") %>%
  #bold(bold = TRUE, part = "header") %>%
  #merge_v(j = c("Year", "FMP", "Coverage category")) %>%
  #hline(i = c(4, 12)) %>%
  #hline(i = c(8, nrow(Pollock.observed.table)), border = officer::fp_border(width = 2)) %>%
  #surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  #fix_border_issues() %>%
  #width(j = 1, 0.5, unit = "in") %>%
  #width(j = 2, 0.84, unit = "in") %>%
  #width(j = c(3, 5:6), 0.81, unit = "in") %>%
  #width(j = 4, 1, unit = "in") %>%
  #width(j = 7, 0.75, unit = "in")
...
flextable(Pollock.observed.table.2022) %>%
  ...
  hline(i = c(4, 8)) %>%
  hline(i = nrow(Pollock.observed.table.2022), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  fix_border_issues() %>%
  width(j = 1, 0.84, unit = "in") %>%
  width(j = c(2, 4:5), 0.81, unit = "in") %>%
  width(j = 3, 1, unit = "in") %>%
  width(j = 6, 0.75, unit = "in")
 
```

#####

####`r tbl_nums('Pollock.observed.table.2023', caption = paste0('The number of pollock deliveries made by vessels in the *OB TRW* stratum during ', year, ', separated by port and coverage category. Trips that delivered to a tender have been excluded. Observed deliveries denote deliveries that were observed shoreside for salmon.'))`  

```{r Pollock.observed.table.2023, echo=FALSE, tab.cap=""}

Pollock.observed.table.2023 <- Pollock.observed.table %>% filter(Year == year) %>% select(-Year)

flextable(Pollock.observed.table.2023) %>%
  mk_par(j = 4, part = "header",
         value = as_paragraph("Total deliveries (", as_i("N"), ")")) %>%
  mk_par(j = 5, part = "header",
         value = as_paragraph("Observed deliveries (", as_i("n"), ")")) %>%
  ...
  merge_v(j = c("FMP", "Coverage category")) %>%
  hline(i = c(4, 9)) %>%
  hline(i = nrow(Pollock.observed.table.2023), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  fix_border_issues() %>%
  width(j = 1, 0.84, unit = "in") %>%
  width(j = c(2, 4:5), 0.81, unit = "in") %>%
...
####`r tbl_nums('Pollock.efp.table.2022', caption = paste0('The number of pollock deliveries made by vessels in the *EM TRW EFP* stratum during ', year - 1, ', separated by port and coverage category. Trips that delivered to a tender have been excluded. Observed deliveries denote deliveries that were observed shoreside for salmon.'))`  

```{r Pollock.efp.table.2022, echo=FALSE, tab.cap=""}
...
#flextable(Pollock.efp.table) %>%
 # mk_par(j = 5, part = "header",
  ...
  #merge_v(j = c("Year", "FMP", "Coverage category")) %>%
  #hline(i = c(5, 14)) %>%
  #hline(i = c(10, nrow(Pollock.efp.table)), border = officer::fp_border(width = 2)) %>%
  #surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  #fix_border_issues() %>%
  #width(j = 1, 0.5, unit = "in") %>%
  #width(j = 2, 0.84, unit = "in") %>%
  #width(j = c(3, 5:6), 0.82, unit = "in") %>%
  #width(j = 4, 1.78, unit = "in") %>%
  #width(j = 7, 0.75, unit = "in")
...
flextable(Pollock.efp.table.2022) %>%
  ...
  merge_v(j = c("FMP", "Coverage category")) %>%
  hline(i = c(5, 10)) %>%
  hline(i = nrow(Pollock.efp.table.2022), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  fix_border_issues() %>%
  width(j = 1, 0.84, unit = "in") %>%
  width(j = c(2, 4:5), 0.82, unit = "in") %>%
  width(j = 3, 1.78, unit = "in") %>%
  width(j = 6, 0.75, unit = "in")
...
####`r tbl_nums('Pollock.efp.table.2023', caption = paste0('The number of pollock deliveries made by vessels in the *EM TRW EFP* stratum during ', year, ', separated by port and coverage category. Trips that delivered to a tender have been excluded. Observed deliveries denote deliveries that were observed shoreside for salmon.'))`  

```{r Pollock.efp.table.2023, echo=FALSE, tab.cap=""}

Pollock.efp.table.2023 <- Pollock.efp.table %>% filter(Year == year) %>% select(-Year)

flextable(Pollock.efp.table.2023) %>%
  mk_par(j = 4, part = "header",
         value = as_paragraph("Total deliveries (", as_i("N"), ")")) %>%
  mk_par(j = 5, part = "header",
         value = as_paragraph("Observed deliveries (", as_i("n"), ")")) %>%
  ...
  merge_v(j = c("FMP", "Coverage category")) %>%
  hline(i = c(4, 9)) %>%
  hline(i = nrow(Pollock.efp.table.2023), border = officer::fp_border(width = 2)) %>%
  surround(part = "header", border.top = officer::fp_border(width = 2), border.bottom = officer::fp_border(width = 2)) %>%
  fix_border_issues() %>%
  width(j = 1, 0.84, unit = "in") %>%
  width(j = c(2, 4:5), 0.82, unit = "in") %>%
...
```{r OBSERVED_FLAG.results, echo=FALSE, tab.cap="", eval=FALSE}
...
```{r outcomes.plot.2022, echo=FALSE, warning=FALSE, fig.width = 7.5, fig.height = 9, dpi = 600}
...
####`r fig_nums('outcomes.plot.2022', caption = paste0('Total number of observer sea days purchased (top panel) and total cost of observing those sea days (bottom panel). Vertical bars signify the range of potential outcomes predicted by the ', year - 1, ' Annual Deployment Plan. Dashed lines signify expected outcomes. Solid lines signify what actually occurred in ', year, '.'))`  

#####

```{r outcomes.plot.2023, echo=FALSE, warning=FALSE, fig.width = 7.5, fig.height = 9, dpi = 600}

arrange.plot.fxn(outcomes.plot.2023, costs.plot.2023)

```

####`r fig_nums('outcomes.plot.2023', caption = paste0('Total number of observer sea days purchased (top panel) and total cost of observing those sea days (bottom panel). Vertical bars signify the range of potential outcomes predicted by the ', year, ' Annual Deployment Plan. Dashed lines signify expected outcomes. Solid lines signify what actually occurred in ', year, '.'))`  

#####

```{r odds.by.date.plot.2022, echo=FALSE, fig.width=7.5, fig.height=8, dpi=600}
...
####`r fig_nums('odds.by.date.plot.2022', caption = paste0('Rate of selected trips logged into ODDS during ', year - 1, ' organized by original date entered for all trips (gray line and gray text), and final date considering only non-canceled trips (black line and black text). The programmed selection rate is depicted as the dotted line. Gray shaded areas denote the range of coverage rate corresponding to the 95% confidence intervals expected from the binomial distribution. The final coverage rates were higher than if trip dates had not been altered and/or canceled. Vertical tick marks on the horizontal axis depict dates when an ODDS trip was selected due to a prior trip being canceled that was selected for observer coverage (gray on the bottom for originally logged trips, and black on the top for trips after user manipulation).'))` 

#####

```{r odds.by.date.plot.2023, echo=FALSE, fig.width=7.5, fig.height=8, dpi=600}
suppressWarnings(print(odds.by.date.plot.2023))   

# Figure 2 in 2019 Annual Report
 
```

####`r fig_nums('odds.by.date.plot.2023', caption = paste0('Rate of selected trips logged into ODDS during ', year, ' organized by original date entered for all trips (gray line and gray text), and final date considering only non-canceled trips (black line and black text). The programmed selection rate is depicted as the dotted line. Gray shaded areas denote the range of coverage rate corresponding to the 95% confidence intervals expected from the binomial distribution. The final coverage rates were higher than if trip dates had not been altered and/or canceled. Vertical tick marks on the horizontal axis depict dates when an ODDS trip was selected due to a prior trip being canceled that was selected for observer coverage (gray on the bottom for originally logged trips, and black on the top for trips after user manipulation).'))` 

#####

```{r em.times.figure, echo=FALSE, warning=FALSE, fig.width = 7.5, fig.height = 6, dpi = 600, eval=FALSE}
...
```{r Temporal.plot.2022, echo=FALSE, warning=FALSE, fig.width=7.5, fig.height=8, dpi=600}
print(Temporal.plot.2022)
```

####`r fig_nums('Temporal.plot.2022', caption = paste0('Cumulative number of trips monitored during ', year - 1, ' (black line) compared to the expected range of observed trips (shaded ribbon) given fishing effort and sampling rates. Dates where the monitored number of trips is outside of expected (less or more than the range) are depicted as tick marks on the horizontal x-axis. The results of tests that the observed rate derived from a binomial distribution sampled at the selection rate are denoted as *p*-values.'))`  

#####

```{r Temporal.plot.2023, echo=FALSE, warning=FALSE, fig.width=7.5, fig.height=8, dpi=600}
print(Temporal.plot.2023)
```

####`r fig_nums('Temporal.plot.2023', caption = paste0('Cumulative number of trips monitored during ', year, ' (black line) compared to the expected range of observed trips (shaded ribbon) given fishing effort and sampling rates. Dates where the monitored number of trips is outside of expected (less or more than the range) are depicted as tick marks on the horizontal x-axis. The results of tests that the observed rate derived from a binomial distribution sampled at the selection rate are denoted as *p*-values.'))`  

#####

```{r effort_monitored_obze_HAL, echo=FALSE, message=FALSE, warning=FALSE, dpi=600, eval=FALSE}
```
...
```{r Obs.perm.plot, echo=FALSE, fig.height=11, fig.width=8, message=FALSE, warning=FALSE, dpi=600, eval=FALSE}
...
```{r freq_geartender_plot, echo=FALSE, fig.height=6, fig.width=6, message=FALSE, warning=FALSE, dpi=600, eval=FALSE}
...
```{r prob.bynumber, echo=FALSE, warning=FALSE, fig.width=6, fig.height=6, dpi=600, eval=FALSE}
