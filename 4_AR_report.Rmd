...
if( !(file.exists("source_data/FMA_Days_Paid.rdata")) ) {
  #' Download the following spreadsheet as a `.csv` format file in the `source_data/` folder. Save an `.rdata` copy to 
  #' the [FMA shared Gdrive].
  ...
  # Upload the shared Gdrive
  gdrive_upload("source_data/FMA_Days_Paid.rdata", gdrive_set_dribble("Projects/AnnRpt-Deployment-Chapter"))
} else {
  #' If the file does exist, make sure you are up-to-date
  gdrive_download("source_data/FMA_Days_Paid.rdata", gdrive_set_dribble("Projects/AnnRpt-Deployment-Chapter"))
}
...
predicted <- predicted %>%
  ...
  rename(pred_days = dh) %>%
  select(!c(POOL, Nh, nh)) %>%
  filter(STRATA != c("HAL", "POT")) # Remove zero strata
...
work.data <- 
  ...
  mutate(work.data, TRIP_TARGET_DATE = as.Date(TRIP_TARGET_DATE), 
         ...
         STRATA = case_match(
           ...
           c("FULL", "ZERO") ~ STRATA,
           .default = "Undefined"),
         ADP = as.factor(ADP) #' *OTHER DATASETS HAVE THIS AS factor, NEEDS TO MATCH DATA TYPE*
         )
...
# Free up workspace
rm(work.data.2022, work.data.2023)
#'*----------------------------------------------------------------------------*
...
trip.budget.table.2022 <- 
  merge(
    ...
    rbind(
      # EM strata
      work.data %>%
      filter(ADP == 2022 & COVERAGE_TYPE == "PARTIAL" & 
...
unique(work.data[ADP == 2023 & LENGTH_OVERALL < 40 & OBSERVED_FLAG == "Y", .(
...
work.data[TRIP_ID %in% c(2170855, 2170858), STRATA := "OB POT"]
work.data[TRIP_ID %in% c(2307113, 2307114), STRATA := "OB HAL"]
...
Coverage.Table.partial <- bind_rows(
  ...
  work.data %>% 
    ...
    left_join(select(partial, c(STRATA, Rate)), by = "STRATA") %>%
    group_by(COVERAGE_TYPE, STRATA) %>%
    summarize(V = length(unique(VESSEL_ID)),
              v = length(unique(VESSEL_ID[OBSERVED_FLAG == "Y"])),
              N = length(unique(TRIP_ID)),
...
Coverage.Table <- bind_rows(Coverage.Table.full, 
  ...
  mutate(V = as.character(V),
         ...
         STRATA = fct_recode(factor(STRATA),
                             Full = "FULL"),
         COVERAGE_TYPE = case_when(STRATA == "Zero coverage" ~ "Zero coverage", TRUE ~ COVERAGE_TYPE),
