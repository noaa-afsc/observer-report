  ...
  rbind(
    work.offload.trw_em %>%
      filter(STRATA == "EM TRW GOA (EFP)") %>%
          ...
          mutate(PORT_CODE = case_when(REPORT_ID == "37252024-09-10" ~ "AKU", #'*2024 AR* incorrect assignment of PORT_CODE for 1 tender delivery
                                       TRUE ~ PORT_CODE)) %>%
          distinct() %>%
          mutate(Observed.type = if_else(Observed.type == "Y", "Observed", "Not_Observed"),
                 COVERAGE_TYPE = case_when(grepl("EM TRW GOA", STRATA) & TENDER == "Y" & COVERAGE_TYPE == "PARTIAL" ~ "Partial (tender)",
