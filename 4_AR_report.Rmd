...
if(year == 2024) {
  ...
  odds.dat[TRIP_PLAN_LOG_SEQ %in% c(202432902, 202427490), 
           ':=' (TRIP_MONITOR_CODE = "RL", RELEASE_COMMENT = "Three-in-a-row", 
                 RELEASE_STATUS_DESCRIPTION = "Three-in-a-row", random.test.result = "PASS")]
  setDF(odds.dat)
}
...
logged.table <- logged %>%
  bind_rows(
    logged %>% 
      ...
      mutate(Pct.selected.from.inherit = round((Inherit.selected / Total.final.selected) * 100, 1),
             Pct.allselected.from.waiver = round(((Waived) / (Total.final.selected + Waived)) * 100, 1))) %>% 
  select("Strata" = STRATA, 
         ...
         "% Selected from inherits ((*i*/*T*) x 100)" = Pct.selected.from.inherit, 
         "% Reduction of selected trips due to waivers (*w*/(*T*+*w*) x 100)" = Pct.allselected.from.waiver) %>% 
  arrange(match(Strata, c("OB HAL", "OB POT", "OB TRW", "EM HAL", "EM POT",  "Total")))
```
...
#'* This object is also used for pollock offload summaries and spatiotemporal analyses *
...
work.data.st.join <- unique(work.offload.trw_em[, .(TRIP_ID, REPORT_ID, OBSERVED_FLAG)])
work.data.st <- copy(work.data) 
work.data.st |>
  # Merge in new TRIP_ID and OBSERVED_FLAG using REPORT_ID
  _[, TRIP_ID_NEW := work.data.st.join[work.data.st, TRIP_ID, on = .(REPORT_ID)]
  ][, OBS_FLAG_NEW := work.data.st.join[work.data.st, OBSERVED_FLAG, on = .(REPORT_ID)]
  # Need TRIP_ID Too stay numeric, so make a helper column
  ][, MAX_TRIP_ID := max(TRIP_ID)
...
rm(work.data.st.join, work.data.st)
...
em_trw_goa_efp.psid <- work.offload.trw_em |>
  _[STRATA == "EM TRW GOA (EFP)", .(TRIP_ID, REPORT_ID, OBSERVED_FLAG)] |>
  unique()
# # Update the values using REPORT_ID
dat.em_trw_goa <- dat[STRATA == "EM TRW GOA (EFP)"]
dat.em_trw_goa[, c("TRIP_ID", "OBSERVED_FLAG") := em_trw_goa_efp.psid [dat.em_trw_goa, .(TRIP_ID, OBSERVED_FLAG), on = .(REPORT_ID)]]
