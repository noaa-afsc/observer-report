...
- Motherships.\
- Catcher vessels participating in programs that have transferable PSC allocations as part of a catch share program **IS THIS NOT THE SAME AS THE LAST BULLET?**.\
...
- More information added to to summary tables (e.g., fishing effort predictions and ODDS)
...
\
Evaluations for the full coverage category and zero-selection pool are straightforward --- either the coverage achieved was equal to 100% or 0%, respectively, or it was not. The program achieved `r Coverage.Table %>% filter(Strata == "Full") %>% pull(Realized) %>% trimws()`% coverage in its *Full coverage* stratum (`r tbl_nums('Coverage.Table', display = "cite")`). One trip was not monitored in the *Full coverage* stratum (**add explanation here**). The program achieved compliance with the *Zero coverage* stratum (`r tbl_nums('Coverage.Table', display = "cite")`).\
\
...
`r tbl_nums('trip.budget.tbl', caption = paste0('Comparison between predicted and actual monitored at-sea days for the partial coverage strata in ', year,'.  Predicted values come from the ', year,' Annual Deployment Plan (ADP).'))`  
