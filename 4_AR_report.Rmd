...
odds.by.date.plot <- ggplot(odds.ts.daily.cuml, aes(x = Date,  col = TYPE)) +
  ...
  geom_text(aes(x = as.Date(Inf), y = Inf, label = paste("Programmed (%) = ", label.programmed, sep = "")), 
            hjust = 1, vjust = 2, size = 3, color = "black", check_overlap = TRUE) +
  geom_text(aes(x = as.Date(Inf), y = Inf, label = paste("Original (%) = ", label.original, sep = "")), 
            hjust = 1, vjust = 3, size = 3, color = "gray70", check_overlap = TRUE) +
  geom_text(aes(x = as.Date(Inf), y = Inf, label = paste("Final (%) = ", label.adjusted, sep = "")), 
            hjust = 1, vjust = 4, size = 3, color = "black", check_overlap = TRUE) +
