...
Pollock.landing.data <- work.data %>%
  ...
  mutate(GEAR = "TRW") %>%
  distinct(REPORT_ID, COVERAGE_TYPE, GEAR, FMP, PORT_CODE, TENDER) %>%
...
Pollock.observed <- rbind(
          ...
          select(FMP, COVERAGE_TYPE, PORT_CODE, N.Pollock, N.Pollock.Obs, Pct.Obs) %>%
          ungroup(),
          Pollock.landing.data %>%
...
# Reorder rows
Pollock.observed <- rbind(
                    filter(Pollock.observed, COVERAGE_TYPE == "Full"),
                    filter(Pollock.observed, COVERAGE_TYPE == "Partial"))
