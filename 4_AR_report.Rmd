...
Coverage.Table.full <- bind_rows(
  ...
  work.data %>%
    ...
    mutate(Pct.obs = (n/N) * 100,
           ...
           #Pct.obs = format(Pct.obs, digits = 1, nsmall = 1)) %>% #' *had to move for logic to work*
...
#' *ZERO COVERAGE* 
Coverage.Table.zero <- #' plyr::rbind.fill(Coverage.Table.zero <- `DO WE NEED THIS LINE?`
  work.data %>%
    ...
    mutate(Pct.obs = (n/N) * 100,
           ...
           #Pct.obs = format(Pct.obs, digits = 1, nsmall = 1)) %>%
    select(COVERAGE_TYPE, STRATA, deployment.method, 
           V, v, N, n, Pct.obs, Rate, meets.expected)
...
Coverage.Table <- bind_rows(Coverage.Table.full, 
                            ...
                            Coverage.Table.zero,
                            #'Coverage.Table.TOTALS # , *THIS TABLE HAS Pct.obs as character*
                            #' Coverage.Table.TOTALS.sansEM *THIS TABLE NO LONGER EXISTS*
                            ) %>%
  mutate(Pct.obs = format(Pct.obs, digits = 1, nsmall = 1)) %>% #' *CAN ELIMINATE ALL THE OTHER format() OF Pct.obs*
