...
#' was less than the trip's selection rate. Since ODDS 3.0 starting in 2023, all trips have `RANDOM_NUMBER_USED` 
...
#' Based on a trip's stratum and the outcome of the trip, flag trips that should have its monitored/unmonitored status 
#' determined by random selection with the `random.test` flag. Trips whose fate was predetermined by prior trips, e.g., 
#' via inherits and cascade cancels OR were undetermined, e.g., system canceled, are excluded with a value of NA. Use 
#' `TRIP_MONITOR_CODE` to identify trips in non-100% compliance monitoring strata that were slated for monitoring, 
#' whether it was realized or not, including those where the trip was waived. Finally, trips in strata without random
#' selection and trips marked as not monitored are identified.
odds.dat <- odds.dat %>% mutate(
  random.test = case_when(
    ...
    !is.na(RELEASE_STATUS_DESCRIPTION) ~ 1,
...
# Check whether the monitoring outcome matches what was determined by random selection
...
#' *GM - I think we can omit this chunk because these tables were last created in 2019*
...
# *In this document, tables 3-2 and 3-3 are from the 2019 Annual Report! The 2020 version of 3-2 is created in chunk selection.tbl, aka 3-4*

#' Before ODDS 3.0, all cancelled (CN or CC) trips had NA for TRIP_SELECTED
#' In 2019, It doesn't look like CS trips were excluded from this table?
#' *NEW*
logged <-
  odds.dat %>% 
    # Exclude 100% compliance trips and cancelled trips
    filter(!ODDS_SELECTION_PCT %in% c(0, 100), !(TRIP_STATUS_CODE %in% c("CN", "CC", "CS", "CR"))) %>%
    group_by(YEAR, STRATA) %>%                                                 # 2024-ONLY: Added YEAR to group
    summarize(
      Total.trips = length(unique(TRIP_PLAN_LOG_SEQ)),
...
logged <-
  ...
  filter(!ODDS_SELECTION_PCT %in% c(0, 100), !is.na(TRIP_SELECTED)) %>%  #' *LOSING ALL THOSE CN TRIPS WITH NA? (GM thinks we are actually INCLUDING cancelled trips that were previously excluded.*
  filter(STRATA %in% partial$STRATA & STRATA != "EM TRW EFP") %>% 
  group_by(YEAR, STRATA) %>%                                                 # 2024-ONLY: Added YEAR to group
...
#' *GM - I think we can omit the rest of this chunk because these tables were last created in 2019. Logged is used in 3-4 though*


# Table 3-3 ---- 
#' *The 2020 document didnt have equivalent 3-2, or 3-3, so we can skip these? They tell us with more granularity how many selected trips were cancelled or waived*
