...
odds.by.date.plot <- 
  ...
  scale_color_manual(name = "Rate Type", values = c("gray70", "black")) +
  ylab("Cumulative coverage percent\n") +
  #coord_cartesian(ylim = c(0, 70)) +
  xlab("\nDate") +
  geom_rug(data = odds.ts.ni.create %>% filter(ni.create > 0), sides = "b", color = "gray70") + 
  ...
  geom_text(aes(x = as.Date(Inf), y = Inf, label = paste("Original (%) = ", label.original, sep = "")), 
            hjust = 1.02, vjust = 3.2, size = 3, color = "gray60", check_overlap = TRUE) +
...
Temporal.plot <- 
  ...
  facet_grid(STRATA ~ YEAR, scales = "free", space = "free_x") +
    # Annotate p-values
