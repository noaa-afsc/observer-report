...
#' *In the 2023 AR* : GM added the 'Waivers.inherited' category, which were previously uncategorized.

logged <- odds.dat %>% 
    # Exclude 100% compliance strata and cancelled trips (only completed or pending trips remain)
    filter(!ODDS_SELECTION_PCT %in% c(0, 100), !(TRIP_STATUS_CODE %in% c("CN", "CC", "CS", "CR"))) %>%
    #' *2023 AR ONLY: Added YEAR to group*
    group_by(YEAR, STRATA) %>%                                                 
    summarize(
      # Count total realized trips (i.e. trips that were not cancelled)
      Total.trips = n(),
      # Count of all trips that were randomly selected (excluding instances where monitoring was inherited)
      Random.selected = sum(random.assign[is.na(INHERIT_TRIP_SEQ) & is.na(RELEASE_STATUS_DESCRIPTION)], na.rm = TRUE), 
      # count of trips where monitoring was inherited and not waived.
      Inherit.selected = length(TRIP_PLAN_LOG_SEQ[!is.na(INHERIT_TRIP_SEQ) & is.na(RELEASE_STATUS_DESCRIPTION)]),            
      # Count of randomly selected trips that were waived.
      Waivers.selected = length(TRIP_PLAN_LOG_SEQ[random.assign == 1 & is.na(INHERIT_TRIP_SEQ) & !is.na(RELEASE_STATUS_DESCRIPTION)]),
      # Count of trips that inherited monitoring but were then waived.
      Waivers.inherited = length(TRIP_PLAN_LOG_SEQ[!is.na(INHERIT_TRIP_SEQ) & !is.na(RELEASE_STATUS_DESCRIPTION)]),
      # Total count of monitored trips
      Total.final.selected = (Random.selected + Inherit.selected) - (Waivers.selected + Waivers.inherited),
      # Percent of total monitored trips due to inherits
      Pct.selected.from.inherit = format(round((Inherit.selected / Total.final.selected) * 100, 1), nsmall = 1),
      # % Reduction in monitored trips due to Waivers
      Pct.allselected.from.waiver = format(round(((Waivers.selected + Waivers.inherited) / (Total.final.selected + Waivers.selected + Waivers.inherited)) * 100, 1), nsmall = 1))


# Table 3-3 ---- 
#' *The 2020 document did not have equivalent 3-2, or 3-3, so we can skip these?*
#' They do tell us with more granularity how many selected trips were cancelled or waived
...
logged.table <- bind_rows(
  ...
  logged %>% 
    ...
    summarise(STRATA = "Total",
            ...
            Waivers.inherited = sum(Waivers.inherited),
  ...
  select("Year" = YEAR, 
         ...
         "Inherited but waived (*n*)" = Waivers.inherited,
...
for.p.wide <- merge(
  logged %>% 
    ...
    summarize(random.n = Random.selected,
              ...
              inherit.butnowaiver.n = (Total.final.selected + Waivers.selected + Waivers.inherited),
              Total.n = Total.trips,
              #' TODO [By Valid trips do we mean completed trips?]
              Source = "Valid trips only"
              ),
  # Now add in the total trips for the random number only
  
  #' TODO [CHECK THAT WE WANT ALL random.assign in here, which includes waived and inherited trips!]
  #' random.n should be the count of rips that were selected randomly by RN, (excluding trips inheriting monitoring!)
  ...
  #   #' *2023 AR ONLY: Added YEAR to group*
  #   group_by(YEAR, STRATA) %>%              
  #   summarize(random.n = length(TRIP_PLAN_LOG_SEQ[random.assign == 1 & !is.na(INHERIT_TRIP_SEQ)]),
  #             Total.n = length(TRIP_PLAN_LOG_SEQ),
...
#' TODO *dplyr::gather has been superceded by pivot_longer()*
p.table <- gather(data = for.p.wide, key, value, -STRATA, -YEAR, -Total.n, -ODDS_SELECTION_PCT, -Source)
...
odds.by.date.plot <- ggplot(odds.ts.daily.cuml, aes(Date, running.pct, col = TYPE)) +
     ...
     geom_text(aes(x = as.Date(Inf), y = 60, label = paste("Programmed (%) = ", label.programmed, sep = "")), 
               hjust = 1, size = 3, color = "black", check_overlap = T) +
     geom_text(aes(x = as.Date(Inf), y = 54, label = paste("Original (%) = ", label.original, sep = "")), 
               hjust = 1, size = 3, color = "gray70", check_overlap = T) +
