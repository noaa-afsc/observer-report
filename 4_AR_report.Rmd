...
In `r year`, the FMA paid for `r formatC(round(ActualYR.2023), format = "d", big.mark = ",")` observer days, which was `r ifelse(eval.2023$pct.budget.remain != 0, abs(eval.2023$pct.budget.remain), " ")` `r ifelse(eval.2023$pct.budget.remain != 0, "%", "")` `r eval.2023$more.or.less` predicted by the average simulation and within the range of possibilities predicted in the `r year` ADP (`r fig_nums('outcomes.plot.2023', display = "cite")`, top panel). Although there was 16.6% less effort in *OB POT* than predicted, *OB HAL* and *OB TRW* both had higher than predicted effort (`r tbl_nums('trip.budget.tbl.2023', display = "cite")`). Despite observing the number of days predicted, expenditures for partial observer coverage were under budget (`r fig_nums('outcomes.plot.2023', display = "cite")`, bottom panel). Cost savings resulted because the cost of a partial coverage observer day in `r year` was less than the expected cost that was estimated in the `r year` ADP.     
...
#' *Not including in the AR since 2019, kept for posterity*, but this tells us with more granularity how many selected 
...
```{r perm.summary.table, echo=FALSE, tab.cap="", eval=F, include=F, eval=FALSE}
# FIXME - disabled this chunk since not EM anymore 
#' TODO *2023 AR - THIS IS OLD CODE - PHIL UPDATED IN SEPARATE SCRIPT AND IS NOW PERMUTATION TEST TABLE*
