...
# Create a version of the partial coverage programmed rate table that includes the waiver period (2020-03-26 through 2020-06-30)  
partial_time <- rbind(
                partial[, Effective_Date := as.Date(Effective_Date)],
                data.table(Effective_Date = as.Date(c("2020-03-26", "2020-03-26")),
                           STRATA = c("HAL", "POT"),
                           Rate = c(0.0000, 0.000),
                           descriptions = c("hook-and-line gear", "pot gear"),
                           formatted_strat = c("*HAL*", "*POT*"),
                           txt = c("0.00% in the *HAL* stratum", "0.00% in the *POT* stratum")))[order(Effective_Date, STRATA)]

# Add expiration date to programmed rates  
partial_time <- partial_time[, .(Effective_Date, Expire_After = as.Date(c("2020-12-31", "2020-12-31", "2020-12-31", "2020-03-25", "2020-03-25", "2020-12-31", "2020-06-30", "2020-06-30", "2020-12-31", "2020-12-31")), STRATA, Rate, descriptions, formatted_strat, txt)]
...
Coverage.Table.partial <- plyr::rbind.fill(
  ...
  work.data %>% 
    ...
    arrange(match(STRATA, c("HAL", "EM HAL", "POT", "EM POT", "TRW", "EM TRW EFP")), Effective_Date, Expire_After)
  ,
...
# Nullify hypothesis test for any time periods with a programmed rate of 0  
  mutate(min.expected = ifelse(Rate == 0, NA, min.expected),
         max.expected = ifelse(Rate == 0, NA, max.expected),
         meets.expected = ifelse(Rate == 0, NA, meets.expected))
...
kbl(Coverage.Table, escape = FALSE) %>% 
  kable_paper() %>% 
  # Group columns
  add_header_above(c(" " = 8, "Coverage" = 2, "95% Confidence interval" = 2, " " = 1))
```
