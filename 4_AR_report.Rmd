...
Pollock.landing.data <- work.data %>%
  ...
  # Check for non-pollock EM trawl offloads
  {
    ...
    non_pollock_count <- as.data.frame(nrow(filtered_data))

    if (non_pollock_count$`nrow(filtered_data)` > 0) {
      cat("\033[33m", paste(non_pollock_count$`nrow(filtered_data)`, "EM trawl offload(s) non-pollock\033[0m")) # yellow
    } else {
