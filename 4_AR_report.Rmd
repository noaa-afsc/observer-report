...
The deployment design for the partial coverage component of the program involves three elements: (1) the selection method to accomplish random sampling; (2) division of the population of partial coverage trips into selection strata; and (3) the allocation of deployment among strata.\
\
...
\
In 2024, NMFS implemented changes to both the stratification definitions and the allocation strategy (*CITE* NMFS 2023 for 2024 ADP). In previous years, strata were defined by the monitoring method (at-sea observer, EM, or none) and the gear type (either hook-and-line, pot, or trawl). The new strata definitions combined hook-and-line and/or pot gear trips together into a single fixed-gear type. Additionally, strata were further defined by the FMP the vessel intended to fish predominantly in - either the Bering Sea or the Gulf of Alaska. The Proximity Allocation algorithm was utilized to determine the sampling rates of the partial coverage strata to reduce data gaps and impacts caused by small sample sizes. As an exception, trips participating in the Pollock Trawl EM Exempted Fishing Permit (EFP) were not included in the allocation algorithm due to its external funding sources for monitoring and its status.\
\
Vessels that join the EM programs may log trips in the EM strata. Initiated by the Council in 2018, vessels that join the fixed-gear EM program use EM equipment to capture location and video information followed by human review for species identifications and counts. Weights for catch estimation are supplied from other sources.\
\
The Pollock Trawl EM EFP began in 2020, completed in 2024, and applied to trawl vessels fishing with pelagic gear targeting walleye pollock (*Gadus chalcogrammus*, hereafter referred to as “pollock”). Vessels participating in this EFP utilized EM equipment to monitor for compliance of maximized retention at-sea and monitoring by observers shoreside during the delivery of catch. Vessels were able to opt-out of the EFP on a trip-by-trip basis and instead log trips in the at-sea observer strata. Pollock Trawl EM became a regulated monitoring program in 2025. Note that unlike other strata that use the trip as the PSU, the Pollock Trawl EM EFP stratum instead defines the PSU as the shoreside delivery, as that is where the observer applies their sample design.\
\
...
- Trips by fixed-gear vessels (using hook-and-line and/or pot gear) greater than or equal to 40 ft length overall (LOA) monitored with at-sea observers predominantly fishing in the Bering Sea (`r strata.rates$label[1]`).\
- Trips by fixed-gear vessels greater than or equal to 40 ft length overall (LOA) monitored with at-sea observers predominantly fishing in the Gulf of Alaska (`r strata.rates$label[2]`).\
- Trips by trawl vessels not participating in the Pollock Trawl EM EFP monitored with at-sea observers predominantly fishing in the Bering Sea (`r strata.rates$label[3]`).\
- Trips by trawl vessels not participating in the Pollock Trawl EM EFP monitored with at-sea observers predominantly fishing in the Gulf of Alaska (`r strata.rates$label[4]`).\
- Trips by fixed-gear vessels listed under the Fixed Gear EM program monitored at-sea by EM systems and predominantly fishing in the Bering Sea (`r strata.rates$label[5]`).\
...
- Shoreside deliveries by trawl vessels listed under the Pollock Trawl EM EFP monitored 100% at-sea by EM systems, monitored shoreside by observers, and fishing in the Gulf of Alaska (`r strata.rates$label[7]` shoreside). Note that vessels fishing under this EFP in the Bering Sea are managed within the full coverage category.\
...
- Improvements to the evaluation of the ODDS, additionally separating trips selected via inheritance from those randomly selected or not selected for monitoring.

# Evaluation of Deployment in `r year`  
The deployment of observers into the `r year` Federal fisheries of Alaska is primarily evaluated at the level of the deployment stratum because each stratum is defined by a different sampling rate or by a different monitoring method (e.g., observers and EM). The ODDS is used to determine the strata of logged trips and randomly select trips for monitoring at the rates prescribed by the `year` ADP. Combined with predictions of fishing effort and monitoring costs, the ADP aims to maximize monitoring within budgetary constraints. However, the monitoring expenditures and number of monitored trips in each stratum that are actually realized may differ from the ADP's projections as a result of discrepancies between the expected and realized fishing effort in each stratum and/or the random nature of the ODDS. This section compares the ADP's expectations with the realized outcomes.\
...
Under a random selection of trips / deliveries the spatiotemporal distribution of monitoring in a stratum should reflect the spatiotemporal distribution of all trips / deliveries in the stratum. The evaluation methods here are adapted from the proximity index described in the `r year` Draft ADP (NMFSa 2023). The proximity index was defined as the proportion of sample units in a stratum that were either monitored or near a monitored sample unit in space or time. By considering sample units that were neither monitored nor neighboring a monitored trip as a gap in monitoring, the proximity index quantifies the spatiotemporal extent of monitoring coverage.\
\
...
\
The spatiotemporal distributions of monitoring coverage in strata were within the expected ranges (the upper, green distributions of Figures `r fig_nums('plt.proximity.stratum', display = "num")` with only one exception. The proximity value of the EM HAL stratum was lower than expected (`r fig_nums('plt.proximity.stratum', display = "cite")`). Therefore, a spatiotemporal bias was apparent in the EM HAL stratum based on the available data. The proximity value for the EM TRW EFP stratum was low but not unexpected, and resulting proximity values were very close to 1 (indicating good spatiotemporal overlap).\
\
The achieved proximity values were within the expected ranges from the sampling rates defined by the ADP (the lower, blue distributions of Figures `r fig_nums('plt.proximity.stratum', display = "num")`) for the *OB HAL*, *OB POT* and *EM TRW EFP* strata. The *OB TRW* stratum had a higher proximity index than expected because it had an unexpectedly high realized monitoring rate. Both fixed-gear EM strata, *EM HAL* and *EM POT*, had lower proximity indices than expected due to the lower than expected realized monitoring rates.
...
`r tbl_nums('trip.budget.tbl', caption = paste0('Comparison between predicted and actual monitored at-sea days for partial coverage strata in ', year,'.  Predicted values come from the ', year,' Annual Deployment Plan (ADP).'))`  
...
`r tbl_nums('Pollock.observed.table', caption = paste0('The number of shoreside pollock deliveries made by catcher vessels in the trawl strata during ', year, ', separated by port and coverage category. Trips that made a delivery to a tender have been excluded. Observed deliveries denote deliveries that were observed shoreside for salmon.'))`  
