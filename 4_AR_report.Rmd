---
title: "Deployment Performance Review of the 2022 North Pacific Observer Program"
...
```{r coverage_paragraph, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r em_times, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
em_times <- EM.review %>% 
...
```{r TEM.Offloads, echo=FALSE, tab.cap=""}

# Identify the number of TEM offloads to be used in a paragraph of Chapter 3

tem.fmp.offloads <- work.data %>% 
  filter(STRATA == 'EM TRW EFP') %>% 
  group_by(FMP) %>% 
  summarize(offloads = n_distinct(REPORT_ID))

tem.fmp.offloads

tem.offloads <- work.data %>% 
  filter(STRATA == 'EM TRW EFP') %>% 
  group_by(ADP) %>% 
  summarize(offloads = n_distinct(REPORT_ID))
...
```{r HAL.waivers, echo=FALSE, tab.cap=""}

# Identify the reason 12 HAL trips were waived -  to be used in a paragraph of Chapter 3

waived <- odds.dat %>% 
  filter(!is.na(WAIVER_TYPE)) %>% 
  filter(DESCRIPTION == 'Declared Gear & Tender Delivery - Longline No Tender Delivery') %>% 
  filter(random.assign == 1)

table(waived$WAIVER_DESCRIPTION)

```

```{r HAL.cancellations, echo=FALSE, tab.cap=""}

# Determine the number of HAL trips not selected for observer coverage and how many of THOSE were cancelled?

hal_not_selected <- odds.dat %>%  
  select(STRATA, random.assign, TRIP_STATUS_CODE, TRIP_PLAN_LOG_SEQ, WAIVER_TYPE, TRIP_PLAN_NUMBER) %>%
  filter(STRATA == 'HAL') %>% 
  mutate(OBS_SELECTION = ifelse(random.assign == 0, 'Not Selected', 'Selected')) %>% 
  group_by(OBS_SELECTION) %>% 
  summarize(Trips.total = n(),
            Trips.cancelled.byusers = length(TRIP_PLAN_LOG_SEQ[TRIP_STATUS_CODE %in% c("CN", "CC")]),) %>% 
  mutate(Pct.cancelled = round((Trips.cancelled.byusers/Trips.total)*100, 3))
