...
load_fee_detail <- "SELECT gf.year, 'GF' AS report_source, 
 ...
 WHERE gf.year = extract(year from sysdate) - 1             -- previous year
   ...
   and gf.fee_person_year_id NOT IN (8080237, 8080239)      -- Fees were waived for Greg Einerson and Gregory Favretto
...
SELECT ifq.year, 'IFQ' AS report_source, 
 ...
 WHERE ifq.year = extract(year from sysdate) - 1             -- previous year
   ...
   and ifq.fee_person_year_id NOT IN (8080237, 8080239)      -- Fees were waived for Greg Einerson and Gregory Favretto"
...
flextable(all_areas) %>% 
  ...
  merge_at(i=16, j=c(1:2)) %>% 
  # Add dashed horizontal lines before gear subtotals:
  border(i = c(3,7,11,14), part = "body", border.bottom = fp_border(style = "dashed")) %>% 
  # Add solid horizontal lines after gear subtotals and add "fix" needed because of cell merges:
  border(i = c(4,8,12,15), part = "body", border.bottom = fp_border(style = "solid")) %>% 
