...
# Location: S:\Observer Program Annual Report\2019_Annual_Report\Chapt2_Fees&Budget\                              #
# Location: C:\Teleworking\Obs Annual Report\Ch 2                                                                 # 
# Objective:                                                                                                      #
#   - Summarize fee liability by area, gear, vessel length, and species for 2019 Annual Report                    #
#   - This code was brainstormed in "2019 Fee Query - DRAFT.sql"                                                  #
...
#                                                                                                                 # 
#   27,329 records for $2,895,377.90                                                                              #
#                                                                                                                 #
# Input:                                                                                                          #
...
#ODBC connection inputs
dsn <- "AKR1_x64"
user <- "ctide"
pw <- "Winter2020"

#Establish connection to ODBC database in order to access CRF database on AKRI:
#channel<-odbcConnect(dsn, user, pw)
...
#Submit SQL query to an ODBC database using the connection established above:
#MAILING_DATE<-sqlQuery(channel,paste(INVOICE_DATE),as.is=TRUE,max=10000000)
#MAILING_DATE$MAIL_DATE2 <- format(as.Date(MAILING_DATE$MAIL_DATE,  "%Y-%m-%d"), "%B %d, %Y")

# Read in .csv file instead of querying the database directly:
MAILING_DATE <- read.csv("MAILING_DATE.csv")
# Convert factor date to useable fomrat for markdown:
MAILING_DATE$MAIL_DATE2 <- format(as.Date(as.character(MAILING_DATE$MAIL_DATE), format = "%d-%b-%y"), "%B %d, %Y")
...
#NOTICE_DATE <- sqlQuery(channel,paste(LATE_NOTICE),as.is=TRUE,max=10000000)

# Read in .csv file instead of querying the database directly:
NOTICE_DATE <- read.csv("NOTICE_DATE.csv")
...
# (S:\Observer Program Annual Report\2019_Annual_Report\Chapt2_Fees&Budget\2019 Fee Query - DRAFT.sql)
# (C:\Teleworking\Obs Annual Report\Ch 2\2019 Fee Query - DRAFT.sql)
LOAD_FEE_DETAIL<-"SELECT gf.year, 'GF' AS report_source, 
 ...
 WHERE gf.year = extract(year from sysdate) - 1             -- previous year
   ...
   --and gf.fee_person_year_id NOT IN (3610441, 3610443, 3610451, 3610585)  -- example of 2018 fee waiver exclusions                                    
...
SELECT ifq.year, 'IFQ' AS report_source, 
       ...
       --       Using MAX(g.gear_code) to identify the only gear on each landing report because catch_report_species_fact has a record for each species 
       --       catch transaction. In the event of a manual landing there may not be a landing report.  Default to HAL in those cases.
       (SELECT case when MAX(g.gear_code) IS NULL then 'HAL' else MAX(g.gear_code) end GEAR 
          ...
          JOIN akfish_report.gear g ON crsf.gear_pk = g.gear_pk 
         WHERE ifq.report_id = crs.el_report_id and crs.expire_date IS NULL) as GEAR_CODE,                                               
       ifq.weight_pounds, ifq.price_per_pound, ifq.fee_amount,  ifq.fee_person_year_id
  FROM akfish_crf.fee_detail_ifq ifq
  JOIN akfish.management_area ma ON ifq.area = ma.id         -- join to management area to translate area IDs
  JOIN akfish.v_vessel v ON ifq.vessel_id = v.id             -- join to the vessel file so we can summarize fees by vessel length
 WHERE ifq.year = extract(year from sysdate) - 1             -- previous year
   and ifq.management_program_id = 12                        -- observer fees"   


#FEE_DETAIL<-sqlQuery(channel,paste(LOAD_FEE_DETAIL),as.is=TRUE,max=10000000)

# Read in .csv file instead of querying the database directly:
FEE_DETAIL <- read.csv("FEE_DETAIL.csv")

# Summarize fee totals for the year:
FEE_TOTAL <- ddply(FEE_DETAIL, .(YEAR), summarize, FEE_TOTAL=sum(as.numeric(FEE_AMOUNT)))
...
Observer coverage for the partial coverage category is funded through a system of fees based on the ex-vessel value of groundfish and halibut, with potential supplements from Federal appropriations. The observer fee is assessed on landings accruing against a Federal total allowable catch (TAC) for groundfish or a commercial halibut quota made by vessels that are subject to Federal regulations and not included in the full coverage category. Therefore, a fee is only assessed on landings of groundfish from vessels designated on a Federal Fisheries Permit or from vessels landing IFQ or CDQ halibut or IFQ sablefish. Within the subset of vessels subject to the observer fee, only landings accruing against the Federal TAC are included in the fee assessment.[^1]

A fee equal to 1.25% of the ex-vessel value is assessed on the landings of groundfish and halibut subject to the fee.  Ex-vessel value is determined by multiplying the standard price for groundfish by the round weight equivalent for each species, gear, and port combination, and the standard price for halibut by the headed and gutted weight equivalent. The standard ex-vessel prices used for `r as.numeric(format(Sys.Date(), "%Y"))-1` fee assessments were published in the *Federal Register* on December 19, 2018 (83 FR 65146).[^2]
...
[^1]:A table with additional information about which landings are and are not subject to the observer fee is in NMFS regulations at 679.55(c) and shown on page 2 of an informational bulletin titled "Observer Fee Collection" on the NMFS Alaska Region website at:[link](https://www.fisheries.noaa.gov/webdam/download/83907745).

[^2]:Available online at:[link](https://www.govinfo.gov/content/pkg/FR-2018-12-19/pdf/2018-27441.pdf). 
...
# Table prep work: combine PTR and NPT into a single TRAWL gear
#FEE_DETAIL$TABLE_GEAR<-ifelse(FEE_DETAIL$GEAR_CODE %in% c('PTR', 'NPT'), 'TRAWL', 
#                              ifelse(FEE_DETAIL$GEAR_CODE == 'HAL', 'HOOK AND LINE', FEE_DETAIL$GEAR_CODE))

# Consolidate Trawl gear codes (NPT and PTR)
FEE_DETAIL$TABLE_GEAR<-ifelse(FEE_DETAIL$GEAR_CODE %in% c('PTR', 'NPT'), 'TRAWL', 
                              ifelse(FEE_DETAIL$GEAR_CODE == 'HAL', 'HOOK AND LINE', as.character(FEE_DETAIL$GEAR_CODE)))


#Table prep work: number the species so they can dcast into the order we want
FEE_DETAIL$SPECIES_ORDER<-ifelse(FEE_DETAIL$SPECIES_GROUP=='Halibut', 'A', 
                                 ifelse(FEE_DETAIL$SPECIES_GROUP=='Sablefish', 'B', 
                                        ifelse(FEE_DETAIL$SPECIES_GROUP=='Pacific Cod', 'C',
                                               ifelse(FEE_DETAIL$SPECIES_GROUP=='Pollock', 'D', 'E'))))


# Summarize fee totals for Table 2-2 (all areas) 
ALL_AREAS<-ddply(FEE_DETAIL, .(YEAR, TABLE_GEAR, LOA_GROUP, SPECIES_ORDER), summarize, FEE=sum(as.numeric(FEE_AMOUNT)))


# Convert data from long forward to wide format and set NAs to 0:
ALL_WIDE<-dcast(ALL_AREAS, TABLE_GEAR + LOA_GROUP ~ SPECIES_ORDER, value.var="FEE")
ALL_WIDE[is.na(ALL_WIDE)]<-0


#Caluculate the 'Vessel length' subtotals:
VL_SUBTOTAL<-ddply(FEE_DETAIL, .(YEAR, TABLE_GEAR, SPECIES_ORDER), summarize, FEE=sum(as.numeric(FEE_AMOUNT)))
VL_SUBTOTAL$LOA_GROUP<-"Gear Subtotal"
VL_SUBTOTAL_WIDE<-dcast(VL_SUBTOTAL, TABLE_GEAR + LOA_GROUP ~ SPECIES_ORDER, value.var="FEE")
VL_SUBTOTAL_WIDE[is.na(VL_SUBTOTAL_WIDE)]<-0



#Bind the vessel length subtotals, all gear subtotals and vessel length data:
WITH_SUBTOTALS<-rbind(ALL_WIDE, VL_SUBTOTAL_WIDE)
WITH_SUBTOTALS[is.na(WITH_SUBTOTALS)]<-0


#Table prep work: number the vessel length categories so they can sort properly in the table:
WITH_SUBTOTALS$VESSEL_ORDER<-ifelse(WITH_SUBTOTALS$LOA_GROUP=='<40',"A",
                                    (ifelse(WITH_SUBTOTALS$LOA_GROUP=='40 - 57.5',"B",
                                            (ifelse(WITH_SUBTOTALS$LOA_GROUP=='>57.5',"C","D")))))

# Sort the table so far by gear and vessel order:
ORDERED<-WITH_SUBTOTALS[order(WITH_SUBTOTALS[,1],WITH_SUBTOTALS[,8]),]


#Calculate 'All Gear' subtotals:
GEAR_SUBTOTAL<-ddply(FEE_DETAIL, .(YEAR, SPECIES_ORDER), summarize, FEE=sum(as.numeric(FEE_AMOUNT)))
GEAR_SUBTOTAL$TABLE_GEAR<-"Total All Gear"
GEAR_SUBTOTAL$LOA_GROUP<-" "
GEAR_SUBTOTAL_WIDE<-dcast(GEAR_SUBTOTAL, TABLE_GEAR + LOA_GROUP ~ SPECIES_ORDER, value.var="FEE")


#Bind the vessel length subtotals, all gear subtotals and vessel length data:
PRE_PRINT<-rbind(ORDERED[,c(1:7)], GEAR_SUBTOTAL_WIDE)
PRE_PRINT[is.na(PRE_PRINT)]<-0

#Calculate 'All Species' subtotals:
PRE_PRINT$TOTAL<-PRE_PRINT[,3] + PRE_PRINT[,4] + PRE_PRINT[,5] + PRE_PRINT[,6] + PRE_PRINT[,7]

# Calculate percentages:
PCT<-PRE_PRINT[PRE_PRINT$TABLE_GEAR=='Total All Gear',]
PCT$TABLE_GEAR<-" "
PCT[,3:8]<-paste(round((PCT[,3:8]/PCT$TOTAL)*100,0), "%", sep="") 

# Round off the decimal places:
PRE_PRINT[,3:8]<-round(PRE_PRINT[,3:8], 0)

# {htmlTable} language="en" (English) uses ',' between every 3 numbers
PRE_PRINT[,3:8] <- txtInt(PRE_PRINT[,3:8], language="en",html=TRUE)

#Add dollar signs
PRE_PRINT$A <- paste0("$", PRE_PRINT$A)
PRE_PRINT$B <- paste0("$", PRE_PRINT$B)
PRE_PRINT$C <- paste0("$", PRE_PRINT$C)
PRE_PRINT$D <- paste0("$", PRE_PRINT$D)
PRE_PRINT$E <- paste0("$", PRE_PRINT$E)
PRE_PRINT$TOTAL <- paste0("$", PRE_PRINT$TOTAL)

#Get rid of the $0 values for readability in the final table
PRE_PRINT[PRE_PRINT=="$0"] <- ""

#Combine the tables
PRE_PRINT2<-rbind(PRE_PRINT, PCT)


#Rename the column headings for printing:
#FOR_PRINT<-rename(PRE_PRINT2, c("TABLE_GEAR"="Gear", "LOA_GROUP"="Vessel Length Category","A"="Halibut", "B"="Sablefish", "C"="Pacific Cod", "D"="Pollock", "E"="All Other Groundfish", "TOTAL"="Total All Species"))

FOR_PRINT <- PRE_PRINT2 %>% 
...
#FOR_PRINT[FOR_PRINT==">57.5"] <- "&gt;57.5"
#FOR_PRINT[FOR_PRINT=="<40"] <- "&lt;40"

FOR_PRINT[FOR_PRINT==">57.5"] <- "GT57.5"
#FOR_PRINT[FOR_PRINT=="<40"] <- "&lt;40"

htmlTable(FOR_PRINT[,2:8],
          ...
          #Number of rows under each table spanner section 
          n.tspanner=c(4, 4, 4, 3, 1, 1),
          padding.tspanner = c("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp"),
          css.tspanner.sep = "border-top: 1px solid;",
          #Each element in this vector corresponds to the formatting in a table spanner total row. If you wanted a line + bolding you'd say "border-top: 1px dashed; font-weight: 900;". Otherwise this formatting is a mystery, so don't change it! ;)
          css.total = c("border-top: 1px dashed;", #This gives dashed divider lines
                        "border-top: 1px dashed;",
                        "border-top: 1px dashed;",
                        "border-top: 1px dashed;",
                        "font-weight: 900;", #this gives bold font
                        "font-weight: 900"),
          rnames=FALSE,
