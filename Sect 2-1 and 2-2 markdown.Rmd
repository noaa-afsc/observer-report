...
working_strata <- fee_strata %>% 
  mutate(SELECTION_POOL = ifelse(is.na(SAMPLING_STRATA_CODE) & LOA_GROUP == '<40', 'No Selection',
                                 ifelse(is.na(SAMPLING_STRATA_CODE) & LOA_GROUP != '<40', 'Observer Trip Selection',
                                        ifelse(SAMPLING_STRATA_CODE %in% c('H', 'P', 'TR'), 'Observer Trip Selection', 
                                               ifelse(SAMPLING_STRATA_CODE %in% c('EM_H', 'EM_P'), 'Fixed-Gear EM',
                                                      ifelse(SAMPLING_STRATA_CODE == 'N', 'No Selection', 
                                                             ifelse(SAMPLING_STRATA_CODE == 'EM_TrawlPartialCoverage', 'Trawl EM', 
                                                                    # I would probably lump the full coverage error as Observer Trip Selection:
                                                                    ifelse(SAMPLING_STRATA_CODE == 'F', 'Error', 'Ack')))))))) %>% 
  mutate(MONITORING = ifelse(SELECTION_POOL %in% c('Observer Trip Selection', 'No Selection'), 'At-Sea Observers', 
                              ifelse(SELECTION_POOL %in% c('Fixed-Gear EM', 'Trawl EM'), 'Electronic Monitoring', 'Other'))) 


# Summarize fees by selection pool and FMP area:
pool_summary <- working_strata %>% 
  ...
  bind_rows(working_strata %>% 
              ...
              summarise(FMP_AREA = 'All Areas',
                        FEE = sum(FEE_AMOUNT), .groups = 'drop')) %>% 
  # Append FMP Area subtotals:
  bind_rows(working_strata %>% 
             group_by(MONITORING, FMP_AREA) %>% 
             summarise(SELECTION_POOL = 'All Pools',
                       FEE = sum(FEE_AMOUNT), .groups = 'drop')) %>% 
  # Append a monitoring pool subtotal:
  bind_rows(working_strata %>% 
              group_by(MONITORING) %>% 
              summarize(SELECTION_POOL = 'All Pools',
                        FMP_AREA = 'All Areas',
                        FEE = sum(FEE_AMOUNT), .groups = 'drop')) %>% 
  ...
  mutate(SORTORDER1 = ifelse(MONITORING == 'At-Sea Observers', 1,
                            ifelse(MONITORING == 'Electronic Monitoring', 2, 
                                   ifelse(MONITORING == 'Other', 3, 4)))) %>%
  mutate(SORTORDER2 = ifelse(SELECTION_POOL == 'Observer Trip Selection', 1,
                             ifelse(SELECTION_POOL == 'No Selection', 2,
                                    ifelse(SELECTION_POOL == 'Fixed-Gear EM', 3, 
                                           ifelse(SELECTION_POOL == 'Trawl EM', 4, 
                                                  ifelse(SELECTION_POOL == 'All Pools', 6, 5)))))) %>% 
...
Table 2-5. -- Observer fees[^9] in `r as.numeric(format(Sys.Date(), "%Y"))-1` by monitoring type, selection pool, and area.[^10]
...
flextable(pool_summary %>% select(MONITORING, SELECTION_POOL, GOA, BSAI, 'All Areas')) %>% 
  # Adjust column header labels where necessary:
  ...
  bold(part = "header") %>% 
  bold(j = 1) %>% 
  bold(i = 7) %>%  #THIS SHOULD BE THE LAST ROW number 
  ...
  border(i = c(3,6), part = "body", border.bottom = fp_border(style = "solid")) %>% 
  fix_border_issues(part = "body") #%>% 
