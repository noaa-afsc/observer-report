...
# Location: S:\Observer Program Annual Report\2023_Annual_Report\Chapt2_Fees&Budget\                              #
#         (this is the same as G:\FMGROUP\Observer Program Annual Report\2023_Annual_Report\Chapt2_Fees&Budget\ ) #
# Location: H:\Observer Program\Annual Report Local GIT Projects\Obs Fee Summary\                                 #
# Objective:                                                                                                      #
#   - Summarize fee liability by area, gear, vessel length, and species for 2023 Annual Report                    #
#   - This code was brainstormed in "2023 Fee Query - DRAFT based on warehouse.sql"                               #
#                                                                                                                 #
#   Note: Do not include administrative fees or interest for late payments.                                       #
#                                                                                                                 # 
#   20,840 records for $4,379,166.                                                                              #
#                                                                                                                 #
# Input:                                                                                                          #
...
# (G:\FMGROUP\Observer Program Annual Report\2023_Annual_Report\Chapt2_Fees&Budget\2023 Fee Query - DRAFT based on warehouse.sql)
load_fee_detail <- "SELECT gf.year, 'GF' AS report_source, 
 ...
 WHERE gf.year = extract(year from sysdate) - 1             -- previous year
   ...
   and gf.catch_report_source_pk != 9778885                 -- Exclude Unisea fees for a landing report (Report ID 9524973) that was deleted ($211.20) - Correction by OMD  
   and gf.fee_person_year_id != 8636521                     -- OMD waived fees ($2.83) for JOHN W HILLSTRAND   
   and gf.report_id NOT IN (8886977, 8888388, 8887979, 8891836) -- Reversal for Nordic Fury/Alyeska being charged $5,470.74 for Pcod landings in voluntary full coverage
...
SELECT ifq.year, 'IFQ' AS report_source, 
 ...
 WHERE ifq.year = extract(year from sysdate) - 1             -- previous year
   ...
   and ifq.fee_person_year_id != 8636521                     -- OMD waived fees ($2.83) for JOHN W HILLSTRAND "
...
# Read in fee data and append the sample strata associated with the landing:
strata_query <- "SELECT gf.year, 'GF' AS report_source, 
       gf.category,
       gf.processor_permit_id as processor_permit,
       gf.vessel_id, gf.adfg_number, --gf.vessel_name, 
       v.length_overall, 
       ...
       g.gear_code,       
       gf.weight_pounds, gf.price_per_pound, gf.fee_amount,  gf.fee_person_year_id, 
       ss.sampling_strata_code, ss.sampling_strata_name
  FROM akfish_crf.fee_detail_gf gf -- The table that contains the detailed records which are subject to fees for GF transactions
  -- Join to the catch_report_species_fact tables in order to pull FMP area and gear information from the ELLR and ELPR:
  JOIN akfish_report.catch_report_species_fact crsf ON gf.catch_report_species_fact_pk = crsf.catch_report_species_fact_pk
  -- Join to the area table to pick up FMP area:
...
SELECT ifq.year, 'IFQ' AS report_source, 
       ...
       NVL((SELECT MAX(g.gear_code) 
             ...
             WHERE ifq.report_id = crs.el_report_id and crs.expire_date IS NULL), decode(ilt.ifq_gear_code, '61', 'HAL', '91', 'POT', '05', 'JIG', '26', 'JIG')) as GEAR_CODE,  --15 and 25?  JIG?                                           
       ifq.weight_pounds, ifq.price_per_pound, ifq.fee_amount,  ifq.fee_person_year_id,
       cr.sampling_strata_code, cr.sampling_strata_name
  FROM akfish_crf.fee_detail_ifq ifq
   -- join to management area to translate area IDs:
  JOIN akfish.management_area ma ON ifq.area = ma.id        
  -- join to the vessel file so we can summarize fees by vessel length:
  ...
  -- join to the PNOL/landing data to pick up gear when there isn't a corresponding landing report (ie out of state landings?):
  JOIN akfish.ifq_landing_transaction ilt ON ifq.confirmation_number = ilt.confirmation_number 
  -- join back to catch report and catch report species fact to pick up sampling strata:
  -- note: there are 15 records with a NULL sampling strata.  Can we figure out what those are?
  LEFT JOIN (SELECT distinct c.el_report_id, ss2.sampling_strata_code, ss2.sampling_strata_name 
               FROM akfish_report.catch_report c 
               JOIN akfish_report.catch_report_source crs ON c.catch_report_source_pk = crs.catch_report_source_pk 
               JOIN akfish_report.catch_report_species_fact crsf2 ON c.catch_report_species_fact_pk = crsf2.catch_report_species_fact_pk
               JOIN akfish_report.sampling_strata ss2 ON crsf2.sampling_strata_pk = ss2.sampling_strata_pk
              WHERE c.year = extract(year from sysdate) - 1 
                AND c.data_source_type_code != 'CEN'
                AND crs.expire_date IS NULL  
                --AND ss2.expire_date IS NULL -- individual gear strata for fixed gear ended in Dec 2023 (so expired now, but not when applied)
                ) cr ON ifq.report_id = cr.el_report_id
 WHERE ifq.year = extract(year from sysdate) - 1             -- previous year
   and ifq.management_program_id = 12                        -- observer fees
   and ifq.fee_person_year_id != 8636521                     -- OMD waived fees ($2.83) for JOHN W HILLSTRAND "
...
# Query the invoice data from database directly:
fee_strata <- dbGetQuery(channel_cas, strata_query) 



# Consider a bit of clean-up (some records don't have a strata OR were incorrectly charged fees):
working_strata <- fee_strata %>% 
  mutate(FEE_STRATA = ifelse(is.na(SAMPLING_STRATA_CODE) & (LOA_GROUP == '<40' | GEAR_CODE == 'JIG') , 'No Selection',
                             ifelse(is.na(SAMPLING_STRATA_CODE) & LOA_GROUP != '<40' & GEAR_CODE == 'HAL', 'Hook and Line',
                                    ifelse(is.na(SAMPLING_STRATA_CODE) & LOA_GROUP != '<40' & GEAR_CODE == 'POT', 'Pot',
                                           ifelse(SAMPLING_STRATA_CODE == 'H', 'Hook and Line',
                                                  ifelse(SAMPLING_STRATA_CODE == 'P', 'Pot',
                                                         ifelse(SAMPLING_STRATA_CODE == 'TR', 'Trawl', 
                                                                ifelse(SAMPLING_STRATA_CODE %in% c('EM_H', 'EM_P'), 'Fixed-Gear EM',
                                                                       ifelse(SAMPLING_STRATA_CODE == 'EM_TrawlPartialCoverage', 'Trawl EM', 
                                                                              ifelse(SAMPLING_STRATA_CODE == 'N', 'No Selection', 'Other')))))))))) %>%
   mutate(SELECTION_POOL = ifelse(FEE_STRATA %in% c('Hook and Line', 'Pot', 'Trawl'), 'Observer Trip Selection',
                                  ifelse(FEE_STRATA == 'No Selection', 'No Monitoring',
                                         ifelse(FEE_STRATA %in% c('Fixed-Gear EM', 'Trawl EM'), 'Electronic Monitoring', 'Other')))) %>% 
   mutate(MONITORING = ifelse(FEE_STRATA %in% c('Hook and Line', 'Pot', 'Trawl'), 'At-Sea Observers',
                              ifelse(FEE_STRATA == 'No Selection', 'No Monitoring',
                                     ifelse(FEE_STRATA %in% c('Fixed-Gear EM', 'Trawl EM'), 'Electronic Monitoring', 'Other')))) 
...
pool_summary <- working_strata %>% 
  ...
  mutate(SORTORDER2 = ifelse(FEE_STRATA == 'Hook and Line', 1,
                              ifelse(FEE_STRATA == 'Pot', 2,
                                     ifelse(FEE_STRATA == 'Trawl', 3,
  ...
  # drop unncessary No Selection subtotal:
  filter(!(SELECTION_POOL == 'No Monitoring' & FEE_STRATA == 'All Strata'))
...
flextable(pool_summary %>% select(MONITORING, SELECTION_POOL, FEE_STRATA, GOA, BSAI, 'All Areas')) %>% 
  # Adjust column header labels where necessary:
  set_header_labels(MONITORING = "Monitoring",
                    SELECTION_POOL = "Selection Pool",
                    FEE_STRATA = 'Strata')  %>% 
  # Format the currency columns:
  colformat_double(j = c(4,5,6), big.mark = ",", prefix = "$", digits = 0) %>% 
  # Bold the headers and monitoring totals:
  bold(part = "header") %>% 
  bold(j = 1) %>% 
  bold(i = 9) %>%  #THIS SHOULD BE THE LAST ROW number 
  # Merge the monitoring labels into a single cell: 
  merge_v(j = 1, part = "body") %>% 
  # Add dashed horizontal lines before monitoring subtotals:
  border(i = c(3,6), part = "body", border.bottom = fp_border(style = "dashed")) %>% 
  # Add solid horizontal lines after monitoring subtotals and add "fix" needed because of cell merges:
  border(i = c(4,7,8), part = "body", border.bottom = fp_border(style = "solid")) %>% 
  fix_border_issues(part = "body") #%>% 
