...
# Location: G:\FMGROUP\Observer Program Annual Report\2023_Annual_Report\Chapt2_Fees&Budget\ )                    #
# Location: H:\Observer Program\Annual Report Local GIT Projects\Obs Fee Summary\                                 #
# Objective:                                                                                                      #
#   - Summarize fee liability by area, gear, vessel length, and species for 2024 Annual Report                    #
#   - This code was brainstormed in "2024 Fee Query - DRAFT based on warehouse.sql"                               #
#                                                                                                                 #
#   Note: Do not include administrative fees or interest for late payments.                                       #
#                                                                                                                 # 
#   18,244 records for $3,496,312.                                                                                #
#                                                                                                                 #
# Input:                                                                                                          #
...
std_price_notice_date <- 'December 27, 2023'

# Federal Register reference for the Federal Register notice for Standard Prices:  
std_price_cfr <- '88 FR 89375'

# Note: URL needs to be updated in footnote 3 below
std_price_notice_url <- 'https://www.federalregister.gov/documents/2023/12/27/2023-28567/fisheries-of-the-exclusive-economic-zone-off-alaska-north-pacific-observer-program-standard'
...
# (G:\FMGROUP\Observer Program Annual Report\2024_Annual_Report\Chapt2_Fees&Budget\2024 Fee Query - DRAFT based on warehouse.sql)
load_fee_detail <- "SELECT gf.year, 'GF' AS report_source, 
 ...
 WHERE gf.year = extract(year from sysdate) - 1             -- previous year
   ...
   and gf.fee_person_year_id NOT IN (9146126,9146234,9146252,9146264,9146292,9146310)   -- OMD waived fees under $5 for 6 people ($13.70 in total)    
   and gf.report_id NOT IN (10129669) -- Correction to Landing Report #10129669 which was seized by OLE and no Observer fee should have been charged. $139.00 is being credited.
...
SELECT ifq.year, 'IFQ' AS report_source, 
       ...
       NVL((SELECT MAX(g.gear_code) 
              ...
              JOIN akfish_report.gear g ON crsf.gear_pk = g.gear_pk 
             WHERE ifq.report_id = crs.el_report_id and crs.expire_date IS NULL), decode(ilt.ifq_gear_code, '61', 'HAL', '91', 'POT', '05', 'HAL', '15', 'HAL', '26', 'JIG')) as GEAR_CODE,  -- 25?  JIG?                                           
 ...
 WHERE ifq.year = extract(year from sysdate) - 1             -- previous year
   ...
   and ifq.fee_person_year_id IS NOT NULL                    -- to only pick up invoiced fees and not ones in final status  
   and ifq.fee_person_year_id NOT IN (9146126,9146234,9146252,9146264,9146292,9146310)   -- OMD waived fees under $5 for 6 people ($13.70 in total)    
   and (ifq.report_id IS NULL OR ifq.report_id != (10129669)) -- Correction to Landing Report #10129669 which was seized by OLE and no Observer fee should have been charged. $139.00 is being credited.
                                                              -- failure to include the 'ifq.report_id IS NULL OR' portion of this code drops other records that shouldn't be dropped "
...
```{r fees_by_program, echo=FALSE, message=FALSE, warning=FALSE}

# Consider a table that summarizes fees by: At-sea observers, Fixed gear EM, trawl EM

# Query strata information for groundfish landings:
strata_query_gf <- "SELECT catch_report_species_fact_pk, ss.sampling_strata_code, ss.sampling_strata_name
                   FROM akfish_report.catch_report_species_fact crsf
                   JOIN akfish_report.sampling_strata ss ON crsf.sampling_strata_pk = ss.sampling_strata_pk
                  WHERE crsf.year_pk = extract(year from sysdate) - 1"

strata_gf <- dbGetQuery(channel_cas, strata_query_gf) 


# Query the strata information for IFQ landings:
strata_query_ifq <- "SELECT distinct crs.el_report_id, fmp.area_code AS fmp, ss.sampling_strata_code, ss.sampling_strata_name, crs.expire_date 
                       FROM akfish_report.catch_report_source crs
                       JOIN akfish_report.catch_report_species_fact crsf ON crs.catch_report_source_pk = crsf.catch_report_source_pk
                       JOIN akfish_report.area fmp ON crsf.fmp_area_pk = fmp.area_pk
                       JOIN akfish_report.sampling_strata ss ON crsf.sampling_strata_pk = ss.sampling_strata_pk
                      WHERE crs.year_pk = extract(year from sysdate) - 1 
                        AND crs.catch_report_type_code = 'ELLR'
                      --AND ss.expire_date IS NULL -- individual gear strata for fixed gear ended in Dec 2023 (so expired now, but not when applied)"

strata_ifq <- dbGetQuery(channel_cas, strata_query_ifq) 


#  Apply sampling strata information to the groundfish fee detail records:   
fee_strata_gf <- fee_detail %>%
  filter(REPORT_SOURCE == 'GF') %>% 
  left_join(strata_gf, by = "CATCH_REPORT_SPECIES_FACT_PK")

# Apply sampling strata information to some of the IFQ fee detail records:
fee_strata_ifq1 <- fee_detail %>%
  filter(REPORT_SOURCE == 'IFQ') %>% 
  left_join(strata_ifq %>% filter(is.na(EXPIRE_DATE)), by = c("REPORT_ID" = "EL_REPORT_ID", "FMP_AREA" = "FMP"))  

# Apply sampling strata information to IFQ fee detail records NOT assigned a strata in previous join: 
fee_strata_ifq2 <- fee_strata_ifq1 %>% 
  filter(is.na(SAMPLING_STRATA_CODE)) %>% 
  select(-c(SAMPLING_STRATA_CODE, SAMPLING_STRATA_NAME)) %>% 
  # Join by report ID and NOT FMP area:
  left_join(strata_ifq %>% select(-c(EXPIRE_DATE, FMP)) %>% distinct(), by = c("REPORT_ID" = "EL_REPORT_ID")) %>% 
  # Hard code strata; in 2025 there are 6 outstanding landings without a strata 
  #      (they either have an expired ELLR or no ELLR and sketchy fish ticket numbers)
  mutate(SAMPLING_STRATA_CODE = ifelse(FISH_TICKET_NUMBER %in% c('GI1 300160'), 'N', SAMPLING_STRATA_CODE),
         SAMPLING_STRATA_CODE = ifelse(FISH_TICKET_NUMBER %in% c('P00 002958', 'P00 002952', 'P00 002953', 'P00 002955', 'P10 002454'), 
                                       'FG_GOA', SAMPLING_STRATA_CODE)) %>% 
  mutate(SAMPLING_STRATA_NAME = ifelse(FISH_TICKET_NUMBER %in% c('GI1 300160'), 'No Selection', SAMPLING_STRATA_NAME),
         SAMPLING_STRATA_NAME = ifelse(FISH_TICKET_NUMBER %in% c('P00 002958', 'P00 002952', 'P00 002953', 'P00 002955', 'P10 002454'), 
                                      'Fixed Gear GOA', SAMPLING_STRATA_NAME))

# Reassemble into a single dataset:
fee_strata <- fee_strata_gf %>% 
  bind_rows(fee_strata_ifq1 %>% filter(!is.na(SAMPLING_STRATA_CODE))) %>% 
  bind_rows(fee_strata_ifq2)


# Assign some new variables:
working_strata <- fee_strata %>% 
  # Drop the CDQ HAL GF full coverage fee from the summary (but footnote it!):
  filter(!SAMPLING_STRATA_CODE == 'F') %>% 
  # Summarize several ways:
  mutate(FEE_STRATA = ifelse(SAMPLING_STRATA_CODE == 'N', 'No Selection',
                             ifelse(SAMPLING_STRATA_CODE %in% c('FG_BSAI', 'FG_GOA'), 'Fixed Gear',
                                    ifelse(SAMPLING_STRATA_CODE %in% c('TR_BSAI', 'TR_GOA'), 'Trawl',
                                           ifelse(SAMPLING_STRATA_CODE %in% c('EM_FG_BSAI', 'EM_FG_GOA'), 'Fixed-Gear EM', 
                                                  ifelse(SAMPLING_STRATA_CODE == 'EM_TR_GOA', 'Trawl EM',  'Other')))))) %>%
  mutate(SELECTION_POOL = ifelse(FEE_STRATA %in% c('Fixed Gear', 'Trawl'), 'Observer Trip Selection',
                                 ifelse(FEE_STRATA == 'No Selection', 'No Selection',
                                        ifelse(FEE_STRATA %in% c('Fixed-Gear EM', 'Trawl EM'), 'EM Subtotal', 'Other')))) %>% 
  mutate(MONITORING = ifelse(FEE_STRATA %in% c('Fixed Gear', 'Trawl'), 'At-Sea Observers',
                             ifelse(FEE_STRATA == 'No Selection', 'No Monitoring',
                                    ifelse(FEE_STRATA %in% c('Fixed-Gear EM', 'Trawl EM'), 'Electronic Monitoring', 'Other')))) 

  

# Summarize fees by strata and FMP area:
pool_summary <- working_strata %>% 
  group_by(MONITORING, SELECTION_POOL, FEE_STRATA, FMP_AREA) %>% 
  summarise(FEE = sum(FEE_AMOUNT), .groups = 'drop') %>% 
  # Append strata 'All Area' subtotals:
  bind_rows(working_strata %>% 
              group_by(MONITORING, SELECTION_POOL, FEE_STRATA) %>% 
              summarise(FMP_AREA = 'All Areas',
                        FEE = sum(FEE_AMOUNT), .groups = 'drop')) %>% 
  # Append Selection Pool subtotals:
  bind_rows(working_strata %>% 
             group_by(MONITORING, SELECTION_POOL, FMP_AREA) %>% 
             summarise(FEE_STRATA = 'All Strata',
                       FEE = sum(FEE_AMOUNT), .groups = 'drop')) %>%  
  # Append Selection Pool 'All Area' subtotals:
  bind_rows(working_strata %>% 
             group_by(MONITORING, SELECTION_POOL) %>% 
             summarise(FEE_STRATA = 'All Strata',
                       FMP_AREA = 'All Areas',
                       FEE = sum(FEE_AMOUNT), .groups = 'drop')) %>%  
  # Append an area subtotal:
  bind_rows(working_strata %>% 
              group_by(FMP_AREA) %>% 
              summarize(MONITORING = 'All Monitoring',
                        SELECTION_POOL = 'All Partial Coverage',
                        FEE_STRATA = 'All Strata',
                        FEE = sum(FEE_AMOUNT), .groups = 'drop')) %>% 
   # Append a total:
  bind_rows(working_strata %>% 
              group_by(.) %>% 
              summarise(MONITORING = 'All Monitoring',
                        SELECTION_POOL = 'All Partial Coverage',
                        FEE_STRATA = 'All Strata',
                        FMP_AREA = 'All Areas',
                        FEE = sum(FEE_AMOUNT), .groups = 'drop')) %>% 
  # Translate into a wide format:
  pivot_wider(names_from = "FMP_AREA", values_from = "FEE", values_fill = 0)  %>% 
  # Sort:
  mutate(SORTORDER1 = ifelse(MONITORING == 'At-Sea Observers', 1,
                            ifelse(MONITORING == 'Electronic Monitoring', 2, 
                                   ifelse(MONITORING == 'No Monitoring', 3, 4)))) %>% 
  mutate(SORTORDER2 = ifelse(FEE_STRATA == 'Fixed Gear', 1,
                             ifelse(FEE_STRATA == 'Trawl', 2,
                                    ifelse(FEE_STRATA == 'Fixed-Gear EM', 1,
                                           ifelse(FEE_STRATA == 'Trawl EM', 2, 
                                                  ifelse(FEE_STRATA == 'No Selection', 1, 4)))))) %>% 
  arrange(SORTORDER1, SORTORDER2) %>% 
  select(-SORTORDER1, -SORTORDER2) %>% 
  # drop unnecessary No Selection subtotal:
  filter(!(SELECTION_POOL == 'No Selection' & FEE_STRATA == 'All Strata')) %>% 
  # Format tweak for subtotals:
  mutate(FEE_STRATA = ifelse(FEE_STRATA == 'All Strata', SELECTION_POOL, FEE_STRATA)) %>%
  select(-SELECTION_POOL)
...
*There is additional text for Section 2-1 (provided by Lisa Thompson with FMA)*




### 2.2	Summaries of Fees Collected from `r as.numeric(format(Sys.Date(), "%Y"))-1`
...
The observer fee equal to 1.65% of the ex-vessel value is assessed on the landings of groundfish and halibut subject to the fee.[^2]  Ex-vessel value is determined by multiplying the standard price for groundfish by the round weight equivalent for each species, gear, and port combination, and the standard price for halibut by the headed and gutted weight equivalent. The standard ex-vessel prices used for `r as.numeric(format(Sys.Date(), "%Y"))-1` fee assessments were published in the *Federal Register* on `r std_price_notice_date` (`r std_price_cfr`).[^3]  Table 2-2, Table 2-3, and Table 2-4 summarize the observer fees that accrued for `r as.numeric(format(Sys.Date(), "%Y"))-1`.  *INSERT SOMETHING SPECIFIC FOR THE YEAR:* Fees are $882,854 lower than the amount assessed in 2023. The decrease in fees is reflected for halibut, sablefish, and Pacific cod landings. While the drop may result from lower standard prices or lower catch, or a combination of the two, some of the decrease in the Bering Sea/Aleutian Islands is from implementation of the Pacific Cod Trawl Cooperative
program, which is in the full coverage category and whose landings are not subject to observer fees. Table 2-5 also summarizes fees by area, but differentiates the type of monitoring the vessel was subject to and which selection pool or strata the vessel was in when the fees accrued. In
`r as.numeric(format(Sys.Date(), "%Y"))-1`, trips with at-sea observers contributed `r paste0('$', prettyNum(pool_summary[pool_summary$FEE_STRATA == 'Observer Trip Selection',"All Areas"], prefix = "$", big.mark = ","))` (`r paste0(round(pool_summary[pool_summary$FEE_STRATA == 'Observer Trip Selection',"All Areas"]/pool_summary[pool_summary$FEE_STRATA == 'All Partial Coverage',"All Areas"]*100,1),'%')`) to the fees; those with electronic monitoring `r paste0('$', prettyNum(pool_summary[pool_summary$FEE_STRATA == 'EM Subtotal',"All Areas"], prefix = "$", big.mark = ","))` (`r paste0(round(pool_summary[pool_summary$FEE_STRATA == 'EM Subtotal',"All Areas"]/pool_summary[pool_summary$FEE_STRATA == 'All Partial Coverage',"All Areas"]*100,1),'%')`), and trips with no monitoring `r paste0('$', prettyNum(pool_summary[pool_summary$FEE_STRATA == 'No Selection',"All Areas"], prefix = "$", big.mark = ","))` (`r paste0(round(pool_summary[pool_summary$FEE_STRATA == 'No Selection',"All Areas"]/pool_summary[pool_summary$FEE_STRATA == 'All Partial Coverage',"All Areas"]*100,1),'%')`). The proportion of fees paid by trips with at-sea observers is a decrease from 2023 (67.1%) and is an increase for trips with electronic monitoring (25.7% in 2023).
...
[^3]:Available online at: [`r std_price_cfr`] (`r std_price_notice_url`). 
...
Table 2-2. -- Observer fees [^4] in `r as.numeric(format(Sys.Date(), "%Y"))-1` by gear, vessel size category, and species or species group for all areas combined.
...
flextable(all_areas) %>% 
  ...
  bold(part = "header") %>% 
  bold(j = 1) %>% 
  bold(i = 14) %>%  #THIS SHOULD BE THE LAST, TOTAL ALL GEAR ROW number 
  # Merge the gear labels into a single cell: 
  merge_v(j = 1, part = "body") %>% 
  # Merge 'Total All Gear' label across 2 cells (gear and vessel length category):
  merge_at(i=14, j=c(1:2)) %>%     #THIS SHOULD BE THE LAST, TOTAL ALL GEAR ROW number 
  # Add dashed horizontal lines before gear subtotals:
  border(i = c(3,6,10,12), part = "body", border.bottom = fp_border(style = "dashed")) %>% 
  # Add solid horizontal lines after gear subtotals and add "fix" needed because of cell merges:
  border(i = c(4,7,11,13), part = "body", border.bottom = fp_border(style = "solid")) %>% 
...
Table 2-3. -- Observer fees[^5] in `r as.numeric(format(Sys.Date(), "%Y"))-1` by gear, vessel size category, and species or species group in the Gulf of Alaska.[^6]
...
Table 2-4. -- Observer fees[^7] in `r as.numeric(format(Sys.Date(), "%Y"))-1` by gear, vessel size category, and species or species group in the Bering Sea/Aleutian Islands.[^8]
...
Table 2-5. -- Observer fees[^9] in `r as.numeric(format(Sys.Date(), "%Y"))-1` by monitoring type, strata or selection pool, and area.[^10]


```{r table4, echo=FALSE}

# Create a table for the Fee Summaries by selection pool:

flextable(pool_summary %>% select(MONITORING, FEE_STRATA, GOA, BSAI, 'All Areas')) %>% 
  # Adjust column header labels where necessary:
  set_header_labels(MONITORING = "Monitoring",
                    FEE_STRATA = 'Strata/Selection Pool')  %>% 
  # Format the currency columns:
  colformat_double(j = c(3,4,5), big.mark = ",", prefix = "$", digits = 0) %>% 
  # Bold the headers and monitoring totals:
  bold(part = "header") %>% 
  bold(j = 1) %>% 
  bold(i = 8) %>%  #THIS SHOULD BE THE LAST ROW number 
  # Merge the monitoring labels into a single cell: 
  merge_v(j = 1, part = "body") %>% 
  # Add dashed horizontal lines before monitoring subtotals:
  border(i = c(2,5), part = "body", border.bottom = fp_border(style = "dashed")) %>% 
  # Add solid horizontal lines after monitoring subtotals and add "fix" needed because of cell merges:
  border(i = c(3,6,7), part = "body", border.bottom = fp_border(style = "solid")) %>% 
  fix_border_issues(part = "body") %>% 
  # Add footer:
  footnote(i=1, j = 4:5, 
           value = as_paragraph("Observer fees (< $2) accidentally charged on a full coverage trip have been excluded from this summary. See section 3.X for additional information."),
           ref_symbols = c("*", "*"),
           part = "header") 
...
[^10]:The Gulf of Alaska includes Pacific halibut regulatory areas 2C, 3A, and 3B; and sablefish regulatory areas Western GOA, Central GOA, West Yakutat, and Southeast Outside. The Bering Sea/Aleutian Islands includes Pacific halibut regulatory areas 4A, 4B, 4C, and 4D; and sablefish regulatory areas Bering Sea and Aleutian Islands.
...
rm(list= ls()[!(ls() %in% c('fee_detail','fee_total', 'all_areas', 'goa', 'bsai', 'fee_strata', 'pool_summary'))])
